...
| 15A | Services, catalogue, clients | 🔄 `in_progress` — data/security/tenancy **foundation built & verified** on PostgreSQL (5 migrations + models + HMAC blind-index contact protection; tenancy coverage 9, contact-protection 7, Pint/Larastan clean); API authorization layer, canonical permission activation, and frontend screens **not yet built** (see Phase 15A section). Not `local_complete`. |
...
- **Lifecycle status:** 🔄 `in_progress` — **foundation built & verified against PostgreSQL 16**. **Not** `local_complete` (frontend, API authorization, and permission activation are not built). **Proof:** [docs/proof/phase-15a.md](proof/phase-15a.md). **Data dictionary:** [services-clients-scheduling.md](architecture/data-dictionary/services-clients-scheduling.md).
- **Exact work completed (verified):**
  - 5 branch-owned migrations (`service_categories`, `services`, `service_personnel_eligibility`, `clients`, `client_consents`) with composite-FK tenant consistency, partial-unique constraints, CHECKs, integer-minor-unit money.
  - 5 enums + 5 models (tenancy traits, ULID keys, `scopeActive`, `Money`, encrypted+masked client contact).
  - Contact protection: `PhoneNumberNormalizer` + `ClientContactIndex` (keyed HMAC blind index; `config/servana.php` + `.env.example` placeholder; never plaintext/logged/returned).
  - `TenantOwnership` registration; 5 factories; `tests/Unit/ClientContactProtectionTest.php`.
- **Tables delivered:** `service_categories`, `services`, `service_personnel_eligibility`, `clients`, `client_consents` (schema only — no routes/UI yet).
- **Routes / permissions / screens delivered:** none yet (pending API + permission + frontend slices).
- **Tests & command results (PostgreSQL, `app` container):** `migrate:fresh` OK; tenancy coverage **9 passed (110 assertions)**; contact protection **7 passed (17 assertions)**; affected coverage/contract suites **14 passed (61 assertions)** (no regression); Pint **447 files PASS**; Larastan level 8 **No errors**.
- **Failures encountered & corrected:** initial `services`/`clients`/`eligibility`/`consents` migrations lacked a `merchant_id`-leading index required by `TenantColumnCoverageTest::hasMerchantIndex`; corrected by adding `index(['merchant_id','branch_id'])` to each (migrations unshipped on this branch — no shipped migration edited).
- **Work skipped & owners (within Phase 15A, pending):** API actions/policies/requests/controllers/resources/routes; canonical permission activation (registry/seed/TS + affected §10.3 test updates); client search; Branch Manager catalogue + HR eligibility + Front Office client/consent screens; §27.1 screen specs + inventory regen; navigation/get-started flips; OpenAPI/TS; Playwright/responsive/dark/axe; Docker image builds.
- **Controlling decisions:** (1) canonical §19.2/19.3 keys to be activated in owning-phase reconciliation — **REM-PERM-001 not closed** (Phase 19 owns full closure); (2) HR (not Branch Manager) owns `personnel.eligibility.manage`; (3) `preferred_personnel_fee_minor` kept internal/non-editable; `preferred_personnel_fee_rules` is Phase 20A.
- **Pending PR/CI/merge:** none opened (per instruction — branch pushed only; no PR until authorized).
- **Residual risk:** phase is not usable until routes/UI land; the permission reconciliation must land with its consuming routes/policies for a coherent §10.3→§19 transition before any completion claim.
- **Context for Phase 15B:** `service_personnel_eligibility` schema + HR management land in 15A; `personnel_availability` and scheduling enforcement are **15B** (do not create `personnel_availability` here).
