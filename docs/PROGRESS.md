...
## Phase 20G — Salary Accrual and Commission Processing (in_progress)
...
- **Lifecycle:** 🚧 `in_progress`. **NOT** `local_complete` / `ci_passed` / `merged` /
  `verified_complete`. Specification-first: no migration until every material gate resolves.
