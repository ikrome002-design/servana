...
| 18B | Payment validation, receipts, refunds, cash-up, period locks | ✅ `local_complete` (pending PR CI/review/merge) — branch `phase-18b-financial-validation-controls`, base `4a489d0` (verified PR #30 merge). All 11 implementation slices done + green and all local gates pass (see `docs/proof/phase-18b.md` §Quality gates): backend **serial 1065 passed / 7 skipped / 5175 assertions** and **parallel 1065 passed / 7 skipped / 5175 assertions** (PG16); Pint clean (877) + Larastan L8 No errors (667); `composer validate --strict` valid; `audit:verify-chain` clean; OpenAPI **152 ops / 130 paths** + TS + contract check OK; frontend **Vitest 222 / 54 files**, ESLint 0 errors, vue-tsc clean, build OK; Playwright four Phase 18B specs **29/0** and full `npm run e2e` **227/0** (360/768/1280 + light/dark + keyboard + axe serious/critical = 0); composer audit no advisories, npm audit high-gate exit 0 (2 moderate transitive `js-yaml` below gate), gitleaks no leaks; php `dev` + nginx `prod` Docker images build. Two defects fixed this checkpoint: **DEF-18B-003** (360px branch-cash-up page overflow → stacked method cards ≤767px / table ≥768px) and **DEF-18B-004** (cash-up component test clock-coupling → derive business date like the component). **REM-PAY-001** stays open until Phase 18B merges with green CI; **REM-PERM-001** stays open (Phase 19). Slices 1–3 done + green: spec-first layer (ADR-0007, data dictionary A–J, 6 state-machine docs); schema substrate (10 forward-only migrations incl. `branch_cash_ups` evolution + `financial_period_locks` exclusion constraint, reversible on PG16, coverage tests + Larastan L8 clean); **group validation + automatic receipt** (`ValidatePaymentRecordingGroup` atomic action, immutable group validation event, coherent component validation, invoice `validated_paid_minor`+state, gap-free receipt numbering, `receipt_pdf` outbox via Phase 10F, per-component 20G commission-handoff seam — 26 tests green; `customer_payment.validate` activated; OpenAPI 111 routes + TS regenerated). Slice 4 done + green: whole-group **rejection / correction request / resubmit / component reference correction** (4 actions, maker≠checker, period-lock-gated, `financial_mutation`; `customer_payment.reject` + `customer_payment.reference_correct` activated; original reference evidence preserved append-only + masked audit; 13 tests + contract/permission/audit coverage green; OpenAPI 115 routes + TS). Slice 5 done + green: **receipts** (list/detail, reissue = new immutable row + new gap-free number referencing the original, authorized signed download via Phase 10F, `receipt.view`/`receipt.reissue` canonical; 18 tests; OpenAPI 119). Slice 6 done + green: **refunds** (request/approve/reject/finalize — external, component-allocated, maker/checker + fresh step-up, non-destructive accounting, per-component 20G reversal handoff) + **finance disputes** (create/review/resolve/reject, invoice/payment linkage, private Phase 10F evidence, source untouched); canonical `refund.create`/`refund.approve`/`refund.finalize`/`finance_dispute.manage` + `RefundApproval` step-up; 18 workflow tests + 40 contract/OpenAPI/step-up green; OpenAPI 131 routes + TS. Slices 7–11 done + green: cash-up + day-close guards (Gate H server-derived expected totals, maker≠checker, `financial_day_close` blockers); DB-backed period locks + exceptional reopen (`423 financial_period_locked` enforced, EXCLUDE overlap, Finance-owns-reopen / MA-approves-exception); finance exports (six supported types, masked CSV, retention/download-count); frontend + 88 §27.1 screen specs + navigation (9 items live + MA period-reopen approvals); responsive/dark/axe proven by the four Playwright specs. |
