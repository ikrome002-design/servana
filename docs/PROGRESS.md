...
| 16A | Appointments | 🔄 `local_complete` — `appointments` table + Front Office appointment machine (create/confirm-via-assign/check-in/reschedule/cancel/cancel-with-reason/no-show) + PostgreSQL double-booking exclusion + branch operating-hours/calendar validation + Phase 15B `PersonnelSchedulingValidator` integration + Branch Manager read-only + Personnel own-scope + canonical `appointment.*` / `personnel.my_appointments.view`; branch `phase-16a-appointments`, base `02f4dc5`. See Phase 16A section. Not `ci_passed`/`merged` (no PR/CI yet). |
| 16B / 16C | Walk-ins & queues / service sessions | ⬜ Not started |
...
## Phase 16A — Appointments (local_complete)

1. **Branch:** `phase-16a-appointments`.
2. **Base commit:** `02f4dc5` (verified Phase 15B merge, PR #25).
3. **Lifecycle status:** 🔄 `local_complete` — full Phase 16A built; all local gates green. **Not** `ci_passed`/`merged`/`verified_complete` (no PR/CI). **Proof:** [docs/proof/phase-16a.md](proof/phase-16a.md).
...
31. **Pending PR/CI/review/merge:** none opened (branch pushed only). CI authoritative for Linux browser/Docker/gitleaks.
...
33. **Phase 16B handoff:** `checked_in→queued` extends the aggregate by expand-and-contract (add status to CHECK, queue-entry action, `queue_entries.appointment_id`); guard queue stub + live busy projection are 16B.
34. **Phase 16C handoff:** `checked_in→in_service` + `service_sessions` extend the aggregate; preferred-personnel execution+fee + commission preview are 16C/20A. **Phase 16B and Phase 16C: Not started.**
