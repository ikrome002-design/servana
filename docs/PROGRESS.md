...
| 17 | Invoicing | 🔄 `local_complete` (in progress) — `invoice_number_sequences` + `invoices` + `invoice_items`; nine-state Merchant-Client Invoice machine; Front Office draft/update + deterministic finalization (gap-free per-merchant numbering + price/preferred-fee/percentage-config snapshots + `financial_mutation` idempotency); Finance additive non-destructive void (request/execute/reject) + adjust; invoice-side period-lock 423 contract; canonical `invoice.*` permission reconciliation; FO+Finance frontend. Full backend 892 pass / 7 skip / 0 fail; Vitest 191; Pint + Larastan L8 + OpenAPI-deterministic(105) + TS parity clean. Branch `phase-17-invoicing`, base `ffe37cc` (verified Phase 16C merge, PR #28). See Phase 17 section. |
...
## Phase 17 — Invoicing (local_complete)

1. **Branch:** `phase-17-invoicing`. **2. Base merge:** `ffe37cc` (verified Phase 16C merge, PR #28).
3. **Lifecycle status:** 🔄 `local_complete` — full Phase 17 built; all local gates green. **Not** `ci_passed`/`merged`/`verified_complete` (no PR/CI). **Proof:** [docs/proof/phase-17.md](proof/phase-17.md).
...
5. **Proof / specs:** [phase-17.md](proof/phase-17.md); data dictionary [invoicing.md](architecture/data-dictionary/invoicing.md); state machine [invoice.md](architecture/state-machines/invoice.md).
...
8. **Final columns:** see [invoicing.md](architecture/data-dictionary/invoicing.md). `invoices` (branch-owned, ULID, 9-state CHECK, additive void/adjust columns, validated_paid_minor Phase-18B-written); `invoice_items` (branch-owned, ULID, service_session_id source); `invoice_number_sequences` (tenant-owned, gap-free counter).
...
30. **Pending PR/CI/review/merge:** committed to `phase-17-invoicing` and pushed; no PR opened. CI is authoritative for the Linux browser/Docker/gitleaks gates. **Phase 18A: Not started.**
