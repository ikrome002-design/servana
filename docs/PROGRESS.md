...
| 16B | Walk-ins & queues | 🔄 `local_complete` — `walk_ins` + `queue_entries` + 8-state Queue Entry machine + appointment `checked_in→queued` expand + atomic walk-in/conversion + deterministic positions/estimator/selector + Front Office board/walk-in/detail + Branch Manager read-only/config + Personnel own-queue + queue permission reconciliation; branch `phase-16b-walk-ins-queues`, base `404fed9` (verified Phase 16A merge, PR #26). All local gates green. **Not** `ci_passed`/`merged` (no PR/CI yet). See Phase 16B section. |
| 16C | Service sessions | ⬜ Not started |
...
## Phase 16B — Walk-Ins and Queues (local_complete)

1. **Branch:** `phase-16b-walk-ins-queues`. **2. Base commit:** `404fed9` (verified Phase 16A merge, PR #26).
3. **Lifecycle status:** 🔄 `local_complete` — full Phase 16B built; all local gates green. **Not** `ci_passed`/`merged`/`verified_complete` (no PR/CI). **Proof:** [docs/proof/phase-16b.md](proof/phase-16b.md).
...
39. **Pending PR/CI/review/merge:** none opened (branch pushed only). CI authoritative for Linux browser/Docker/gitleaks.
