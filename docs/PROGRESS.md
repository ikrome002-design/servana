...
| 18A | Merchant-client payment recording (Correction 18) | 🔄 `local_complete` — branch `phase-18a-payment-recording`, commit `baa3678` (pushed, NO PR), base `6557469` (verified Phase 17 merge, PR #29). 4 tables + domain + HTTP + FO/Finance frontend; payment 62 tests, full parallel 952 pass/7 skip/0 fail; Larastan L8/Pint/OpenAPI(110)/TS/build/vitest 196 clean. **Not** `ci_passed`/`merged`/`verified_complete`. See Phase 18A section. |
| 18B | Payment validation, receipts, refunds, cash-up, period locks | ⬜ Not started |
...
## Phase 18A — Merchant-Client Payment Recording (local_complete)

1. **Branch:** `phase-18a-payment-recording`. **2. Base merge:** `6557469` (verified Phase 17 squash merge, PR #29; `git fsck` exit 0, only dangling blobs).
3. **Lifecycle status:** 🔄 `local_complete` — full Phase 18A built; all local gates green; committed `baa3678` and pushed. **Not** `ci_passed`/`merged`/`verified_complete` (no PR/CI). CI is authoritative for the Linux browser/Docker/gitleaks gates.
...
35. **Pending PR/CI/review/merge:** committed to `phase-18a-payment-recording`; no PR opened. CI authoritative for Linux browser/Docker/gitleaks.
36. **Residual risks:** local Windows Playwright not claimed (Linux CI authoritative); concurrency proven via the DB partial-unique + invoice row lock + savepoint (sequential tests + index behavior) rather than true parallel processes; item-level allocation is a nullable 18B seam.
