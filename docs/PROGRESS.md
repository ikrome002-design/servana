...
| UI-08 | Super Administrator experience (22 pages) | 🟡 `local_complete` pending PR CI/review/merge | `phase-ui-08-super-administrator-experience` — **one atomic commit** off base `16d544c5`, pushed, **no PR**. All 12 increments complete. **Super Administrator 17 implemented / 5 `disabled_by_gate` / 0 planned / 0 removed = 22**; global contract **160**; the other seven account contracts semantically unchanged (field-by-field diff). `UI08-AMBIG-001` resolved by product-owner decision **`COR-UI08-001`**, which delivered four corrective backend domains (SMS billing, subscription operations read-only, internal platform access, feature flags) — **33 operations, 8 tables + 1 expand migration**, plus one separately itemised read `platform.dashboard.show`: **OpenAPI 302 → 336**. Exactly **two** permission keys added (`platform.internal_access.view` / `.manage`) — **169 / 134 / 35**. The router is now built per account host (`createAppRouter(accountKey)`), which is what allowed the canonical paths to be registered at all: `/audit`, `/dashboard`, `/account` and `/reports` are contract routes for more than one account. **17 canonical routes + 4 compatibility redirects, no `/platform/dashboard` redirect, `/platform` retained as the role landing.** Gates: backend serial **2,967 / 14 skipped / 0 failed / 48,759 assertions** and parallel identical; permission suite 103; Pint 1,876 clean; Larastan L8 0 errors; ESLint 0 errors; typecheck clean; Vitest **129 files / 1,352**; build clean; focused UI-08 browser **79 / 0 / 0 / 0**; full Playwright **1,172 / 0 failed / 0 flaky / 0 skipped**; gitleaks no leaks; `git fsck` 0 errors. Historical evidence: 237 frozen files byte-identical, 66 generated projections verified by `nav:check` instead. Defects closed locally: `UI08-API-001`, `UI08-API-002`, `UI08-NAV-001`, `UI08-NAV-002`, `UI08-NAV-003`, `UI08-A11Y-001`, `UI08-RESP-001`, `UI08-AUDIT-001`, `UI08-EXPORT-001`, `UI08-E2E-001`, `UI08-E2E-002`, `UI08-ROUTE-002`, `UI08-ROUTE-003`, `UI08-RENDER-001`. | [ui-08.md](proof/ui-08.md) | UI-07 merged |
...
Review the pushed `phase-ui-08-super-administrator-experience` branch, create the UI-08 pull request
into `main`, allow the five required checks and the governance/review process to complete, merge,
then reconcile UI-08 and its locally closed defects to `verified_complete` before beginning UI-09.

Phase UI-07 is now `verified_complete` (PR #57 merged as `16d544c5`), and its closures
`UI07-GUARD-001`, `UI07-GUARD-002`, `UI07-ROUTE-001` and `UI07-NAV-001` are promoted with it.
`UI07-ENV-001` remains open as a test-determinism residual, honoured rather than worked around.
