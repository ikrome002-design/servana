...
| UI-10 | Branch experience (18 pages) | 🟠 `local_complete` pending replacement PR CI / governance / merge | `phase-ui-10-branch-experience` from verified UI-09 squash `84b7f803…`; implementation commit `c73d1be…`; non-draft PR #60. Final **15 implemented / 3 disabled_by_gate / 0 planned / 0 removed = 18**; OpenAPI **337→341**; permissions **169/134/35** (delta 0/0). Local gates: Pint 1,892; Larastan 1,448/0; PostgreSQL **2,976 passed / 14 skipped / 48,859 assertions**; Vitest **1,371**; dedicated browser **52/52**; whole-product browser **1,325/1,325**; production host **38/38**; security/integrity green. Initial exact-head run `31665290449`: Docker/Security/E2E succeeded; Frontend and Backend both named only stale UI-06 `public-route-matrix.json` (Backend otherwise 3,541 passed / 5 skipped / 51,545 assertions). UI10-CI-001 regenerates that current projection and excludes only it from frozen restoration. Eleven closures are `local_complete`. | [ui-10.md](proof/ui-10.md) | Next: narrow correction commit, replacement five-job exact-head CI/governance/squash/cleanup; UI-11/Phase 25 not started; Gate W closed |
