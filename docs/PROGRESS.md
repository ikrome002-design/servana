...
| 21R-A | Citrus Refer & Earn — referral capture, outbox, signed delivery | 🚧 `local_complete pending PR CI/review/merge` — Increments 1–7 COMPLETE + green; one completion commit; branch pushed; **no PR yet** (separate product-owner authorization required). Branch `phase-21r-a-referral-capture-outbox` off `6047835…` (Phase 20H PR #43 squash merge). Gate W **CLOSED** (`docs/integrations/wallet/gate-w-evidence.md` absent) → 20D-W stays blocked → 21R-A is the next executable phase (Plan §80.1: `20B → 21R-A`, parallel-eligible with 20C–20E; 20B merged in PR #36). See the Phase 21R-A section + `docs/proof/phase-21r-a.md`. |
...
## Phase 21R-A — Citrus Refer & Earn Referral Capture, Outbox, Signed Delivery (local_complete pending PR)
...
- **Lifecycle:** 🚧 **`local_complete pending PR CI/review/merge`** — Increments 1-7 all COMPLETE and
  green; one completion commit created; branch pushed; **no PR exists** (opening it and observing CI
  needs separate product-owner authorization). Branch `phase-21r-a-referral-capture-outbox` created off
...
- **Pending after Phase 21R-A:** open the PR, observe CI, merge, delete the branch, then reconcile
  21R-A to `verified_complete` on the next phase's branch. Phase 21R-B stays blocked behind 20D-W /
  Gate W; **21R-A does not unblock it on its own**.
