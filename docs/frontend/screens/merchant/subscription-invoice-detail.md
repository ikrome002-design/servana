# Screen specification — Subscription invoice detail

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **UI-09**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `subscription-invoice-detail`
- **Route name and URL:** `merchant.subscription-invoice-detail`
- **Layout:** `MerchantLayout`
- **Allowed roles:** `merchant_administrator`
- **Required permissions:** `merchant.subscription.invoice.view`, `merchant.subscription.invoice.download` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Immutable merchant-to-Servana invoice snapshot with private PDF generation/download and an exact External Gate W treatment in place of payment initiation or success.
