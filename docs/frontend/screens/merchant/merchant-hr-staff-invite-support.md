# Screen specification — Invite HR staff
...
- **Screen key:** `merchant-hr-staff-invite-support`
- **Route name and URL:** `merchant.hr-invitations`
- **Layout:** `MerchantLayout`
- **Allowed roles:** `merchant_administrator`
- **Required permissions:** `staff.invite` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Narrow Merchant Administrator supporting route for the shipped HR invitation authority; it does not register the HR account tree.
