...
- **Screen key:** `merchant-platform-fees`
- **Route name and URL:** `merchant.platform-fees`
- **Layout:** `MerchantLayout`
- **Allowed roles:** `merchant_administrator`
- **Required permissions:** `platform_fee.view`, `platform_fee.dispute` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Merchant-wide masked platform-fee visibility (server earned summary per currency; fee-entry list/detail with ULID references, canonical tier label, rate and integer amounts, status) plus a dispute list and capability-gated create. Server-side merchant scope; no configuration, review, or reversal controls. Amounts are server-authoritative and the browser never recomputes fee money. No Wallet/settlement UI.
