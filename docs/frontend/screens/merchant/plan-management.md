...
- **Fields and displayed data:** Available plans + effective prices, current plan, schedule/cancel a NO-PRORATION next-cycle change with a server-computed effective date (no client-supplied/immediate change). Mutation controls removed in billing read-only; EnsureBillingMutable is authoritative. Structured 409/422 surfaced.
