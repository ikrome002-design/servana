...
- **Screen key:** `subscription-dashboard`
- **Route name and URL:** `merchant.subscription`
- **Layout:** `MerchantLayout`
- **Allowed roles:** `merchant_administrator`
- **Required permissions:** `merchant.subscription.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Subscription lifecycle status + independent merchant billing status, current plan/price, billing interval, trial + current-period dates, pending scheduled change, latest invoice summary, and a billing read-only explanation. Read-only; backend authoritative.
