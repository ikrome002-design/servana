...
- **Screen key:** `merchant-staff-overview`
- **Route name and URL:** `merchant.staff`
- **Layout:** `MerchantLayout`
- **Allowed roles:** `merchant_administrator`
- **Required permissions:** `branches.manage_users_lifecycle` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Paginated tenant-wide safe membership projection with Branch Manager/HR invitation, lifecycle impact confirmation, session counts and branch/status history; phone and client data are absent.
