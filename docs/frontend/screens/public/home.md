# Screen specification — Public account landing page

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase UI-06**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `home`
- **Route name and URL:** `home`
...
- **Fields and displayed data:** The resolved account host's own public landing page: sixteen semantic regions built from that account's compiled content, curated imagery, approved factual trust evidence, role-appropriate plan access and registry-derived calls to action.
