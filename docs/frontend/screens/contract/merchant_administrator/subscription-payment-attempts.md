# Screen specification — Billing Payment Attempts
...
- **Host:** `servana.ke`
- **Page title:** Billing Payment Attempts
- **Route:** `/subscription/payment-attempts` (host-relative contract path)
- **Route name:** `merchant.subscription-payment-attempts`
- **Navigation group:** Subscription & Billing
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.subscription-payment-attempts`
- **Screen key:** `subscription-payment-attempts`
- **Authoritative map section:** §6.4.12

## Purpose

- **Purpose:** Let the merchant owner inspect tenant-level subscription payment attempt progress and outcomes.
- **User story:** As merchant administrator, I open Billing Payment Attempts so that let the merchant owner inspect tenant-level subscription payment attempt progress and outcomes.
