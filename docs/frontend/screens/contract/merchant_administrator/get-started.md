...
>
> A real runtime route renders this page today: `merchant.get-started` at `/merchant/get-started` (routes/merchant.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/get-started`; owner phase **UI-09** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `merchant.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §6.4.3

## Purpose

- **Purpose:** Provide a resumable owner checklist after mandatory setup and help the merchant reach a usable operating state.
- **User story:** As merchant administrator, I open Get Started so that provide a resumable owner checklist after mandatory setup and help the merchant reach a usable operating state.
...
- **Runtime route:** `merchant.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `merchant.get-started` (recorded in `docs/frontend/screens/merchant/merchant-get-started.md`).
- **Data fields:** Guided get-started checklist for merchant setup.
