# Screen specification — High-Value Payout Approvals

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `merchant.compensation-payout-approvals` at `/compensation/payout-approvals` (routes/merchant.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_administrator
- **Host:** `servana.ke`
- **Page title:** High-Value Payout Approvals
- **Route:** `/compensation/payout-approvals` (host-relative contract path)
- **Route name:** `merchant.compensation-payout-approvals`
- **Navigation group:** Compensation & Approvals
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.compensation-payout-approvals`
- **Screen key:** `compensation-payout-approvals`
- **Authoritative map section:** §6.4.19

## Purpose

- **Purpose:** Allow the owner to approve or reject payout runs that exceed the configured high-value threshold.
- **User story:** As merchant administrator, I open High-Value Payout Approvals so that allow the owner to approve or reject payout runs that exceed the configured high-value threshold.
...
- **Runtime route:** `merchant.compensation-payout-approvals`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `merchant.compensation-payout-approvals` (recorded in `docs/frontend/screens/merchant/merchant-payout-approvals.md`).
- **Data fields:** Dedicated high-value approval queue using the existing fresh-step-up, idempotency, maker/checker and audit authority. No payout preparation, verification or mark-paid controls.
...
- **Permission-any:** — none
- **Permission-all:** `merchant.payout.approve_high_value`
