# Screen specification — Subscription Dashboard

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `merchant.subscription` at `/merchant/subscription` (routes/merchant.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/subscription`; owner phase **UI-09** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route:** `/subscription` (host-relative contract path)
- **Route name:** `merchant.subscription`
- **Navigation group:** Subscription & Billing
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.subscription`
- **Screen key:** `subscription`
- **Authoritative map section:** §6.4.8
...
- **User story:** As merchant administrator, I open Subscription Dashboard so that provide the merchant owner with the single authoritative view of plan, billing lifecycle, charges, payment, and recovery.
...
- **Runtime route:** `merchant.subscription`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `merchant.subscription` (recorded in `docs/frontend/screens/merchant/subscription-dashboard.md`).
- **Data fields:** Subscription lifecycle status + independent merchant billing status, current plan/price, billing interval, trial + current-period dates, pending scheduled change, latest invoice summary, and a billing read-only explanation. Read-only; backend authoritative.
...
- **Permission-any:** — none
- **Permission-all:** `merchant.subscription.view`
