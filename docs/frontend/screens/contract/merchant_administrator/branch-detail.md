# Screen specification — Branch Detail

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `merchant.branch-detail` at `/branches/:branchUlid` (routes/merchant.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_administrator
- **Host:** `servana.ke`
- **Page title:** Branch Detail
- **Route:** `/branches/:branchUlid` (host-relative contract path)
- **Route name:** `merchant.branch-detail`
- **Navigation group:** Merchant › Branches
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.branch-detail`
- **Screen key:** `branch-detail`
- **Authoritative map section:** §6.4.6
...
- **User story:** As merchant administrator, I open Branch Detail so that provide tenant-owner oversight for one branch without taking over Branch Manager, HR, Front Office, or Finance functions.
...
- **Runtime route:** `merchant.branch-detail`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `merchant.branch-detail` (recorded in `docs/frontend/screens/merchant/merchant-branch-detail.md`).
- **Data fields:** Scoped owner oversight for one branch with explicit Branch Manager, HR, Front Office and Finance responsibility boundaries and no operational override.
