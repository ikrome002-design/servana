# Screen specification — Subscription Invoice Detail and Payment

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `merchant.subscription-invoice-detail` at `/subscription/invoices/:invoiceUlid` (routes/merchant.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_administrator
- **Host:** `servana.ke`
- **Page title:** Subscription Invoice Detail and Payment
- **Route:** `/subscription/invoices/:invoiceUlid` (host-relative contract path)
- **Route name:** `merchant.subscription-invoice-detail`
- **Navigation group:** Subscription & Billing › Subscription Invoices
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.subscription-invoice-detail`
- **Screen key:** `subscription-invoice-detail`
- **Authoritative map section:** §6.4.11

## Purpose

- **Purpose:** Explain one invoice and provide the full Wallet-orchestrated M-Pesa payment workflow.
- **User story:** As merchant administrator, I open Subscription Invoice Detail and Payment so that explain one invoice and provide the full Wallet-orchestrated M-Pesa payment workflow.
...
- **Runtime route:** `merchant.subscription-invoice-detail`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `merchant.subscription-invoice-detail` (recorded in `docs/frontend/screens/merchant/subscription-invoice-detail.md`).
- **Data fields:** Immutable merchant-to-Servana invoice snapshot with private PDF generation/download and an exact External Gate W treatment in place of payment initiation or success.
...
- **Permission-any:** — none
- **Permission-all:** `merchant.subscription.invoice.view`
...
- **Icon:** `invoice` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
