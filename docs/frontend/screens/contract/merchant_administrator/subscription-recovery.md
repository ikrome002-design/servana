# Screen specification — Billing Recovery

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> Blocked by **external_gate_w**. The navigation entry is rendered disabled and names the gate; it has no live destination, and no Wallet, Refer & Earn, notification or provider runtime exists behind it. Owner phase **UI-09** implements the page once the gate opens.

## Identity

- **Account:** merchant_administrator
- **Host:** `servana.ke`
- **Page title:** Billing Recovery
- **Route:** `/subscription/recovery` (host-relative contract path)
- **Route name:** `merchant.subscription-recovery`
- **Navigation group:** Subscription & Billing
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_administrator.subscription-recovery`
- **Screen key:** `subscription-recovery`
- **Authoritative map section:** §6.4.13

## Purpose

- **Purpose:** Provide the only operational recovery path during read-only grace or billing suspension.
- **User story:** As merchant administrator, I open Billing Recovery so that provide the only operational recovery path during read-only grace or billing suspension.
...
- **Icon:** `recovery` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
