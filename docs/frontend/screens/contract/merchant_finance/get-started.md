...
>
> A real runtime route renders this page today: `finance.get-started` at `/finance/get-started` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/get-started`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `finance.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §9.4.2

## Purpose

- **Purpose:** Teach the Finance user the exact checker workflows and controls.
- **User story:** As merchant finance, I open Get Started so that teach the Finance user the exact checker workflows and controls.
...
- **Runtime route:** `finance.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.get-started` (recorded in `docs/frontend/screens/finance/finance-get-started.md`).
- **Data fields:** Guided get-started checklist for Finance.
