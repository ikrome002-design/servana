...
>
> A real runtime route renders this page today: `finance.settings` at `/settings` (routes/finance.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** Finance Settings and Security
- **Route:** `/settings` (host-relative contract path)
- **Route name:** `finance.settings`
- **Navigation group:** Utility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.settings`
- **Screen key:** `settings`
- **Authoritative map section:** §9.4.24
...
- **User story:** As merchant finance, I open Finance Settings and Security so that manage Finance user preferences, MFA, permitted workflow defaults, and own sessions without changing merchant-wide financial policy.
...
- **Runtime route:** `finance.settings`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.settings` (recorded in `docs/frontend/screens/finance/finance-settings.md`).
- **Data fields:** Finance user's own identity, MFA, sessions, theme, and assigned-branch security context; no merchant configuration, payment-provider credentials, subscription controls, or financial-authority changes.
...
- **Icon:** `preferences` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
