...
>
> A real runtime route renders this page today: `finance.receipts` at `/finance/receipts` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/receipts`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).
...
- **Page title:** Receipts
- **Route:** `/receipts` (host-relative contract path)
- **Route name:** `finance.receipts`
- **Navigation group:** Merchant-Client Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.receipts`
- **Screen key:** `receipts`
- **Authoritative map section:** §9.4.10

## Purpose

- **Purpose:** Review automatically issued receipts and perform permissioned reissue.
- **User story:** As merchant finance, I open Receipts so that review automatically issued receipts and perform permissioned reissue.
...
- **Runtime route:** `finance.receipts`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.receipts` (recorded in `docs/frontend/screens/finance/finance-receipts.md`).
- **Data fields:** Finance receipt list (branch-scoped): receipt number, invoice, server-formatted KES amount, reissue badge, and readiness. Receipts are issued AUTOMATICALLY on validation; there is no manual issue action.
...
- **Icon:** `receipt` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
