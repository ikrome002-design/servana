# Screen specification — Subscription Payment Attempts
...
- **Page title:** Subscription Payment Attempts
- **Route:** `/subscription/payment-attempts` (host-relative contract path)
- **Route name:** `finance.subscription-payment-attempts`
- **Navigation group:** Subscription Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.subscription-payment-attempts`
- **Screen key:** `subscription-payment-attempts`
- **Authoritative map section:** §9.4.19

## Purpose

- **Purpose:** Investigate Wallet-orchestrated attempt lifecycle and application state.
- **User story:** As merchant finance, I open Subscription Payment Attempts so that investigate Wallet-orchestrated attempt lifecycle and application state.
...
- **Tests:** Contract-level only in UI-07: `Ui07NavigationRegistryContractTest`, `Ui07NoPlannedRouteExposureTest`. Page-level tests are owned by **UI-12**.
- **Screenshot requirements:** None in UI-07 — there is no page to capture. Owner phase **UI-12**.
