...
>
> A real runtime route renders this page today: `finance.cash-up` at `/finance/cash-up` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/cash-up`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** Cash-Up and Reconciliation
- **Route:** `/cash-up` (host-relative contract path)
- **Route name:** `finance.cash-up`
- **Navigation group:** Controls & Close
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.cash-up`
- **Screen key:** `cash-up`
- **Authoritative map section:** §9.4.13

## Purpose

- **Purpose:** Review Branch Manager cash-up submissions and approve, reject, or request correction.
- **User story:** As merchant finance, I open Cash-Up and Reconciliation so that review Branch Manager cash-up submissions and approve, reject, or request correction.
...
- **Runtime route:** `finance.cash-up`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.cash-up` (recorded in `docs/frontend/screens/finance/finance-cash-up.md`).
- **Data fields:** Finance cash-up review inbox (branch-scoped): business date, server-derived expected + counted totals, and variance per submitted cash-up. Opens the review detail.
