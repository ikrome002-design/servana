# Screen specification — Finance Audit Activity

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `finance.audit` at `/finance/audit` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/audit`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** Finance Audit Activity
- **Route:** `/audit` (host-relative contract path)
- **Route name:** `finance.audit`
- **Navigation group:** Reporting & Audit
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.audit`
- **Screen key:** `audit`
- **Authoritative map section:** §9.4.22

## Purpose

- **Purpose:** Review branch finance events and maker/checker evidence.
- **User story:** As merchant finance, I open Finance Audit Activity so that review branch finance events and maker/checker evidence.
...
- **Runtime route:** `finance.audit`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.audit` (recorded in `docs/frontend/screens/finance/finance-audit.md`).
- **Data fields:** The Finance role's own MFA-gated, branch-scoped, masked read of the finance-domain audit trail (finance.audit.view; distinct from the Audit role's audit.finance.view). Read-only: no operational mutation and no flagged-review controls; MFA is enforced by the API and reflected in the UI.
...
- **Permission-any:** — none
- **Permission-all:** `finance.audit.view`
