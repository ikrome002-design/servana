# Screen specification — Duplicate Reference Review

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `finance.payments-duplicates` at `/payments/duplicates` (routes/finance.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** Duplicate Reference Review
- **Route:** `/payments/duplicates` (host-relative contract path)
- **Route name:** `finance.payments-duplicates`
- **Navigation group:** Merchant-Client Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.payments-duplicates`
- **Screen key:** `payments-duplicates`
- **Authoritative map section:** §9.4.6

## Purpose

- **Purpose:** Investigate suspected duplicate offline payment references and control exceptional override.
- **User story:** As merchant finance, I open Duplicate Reference Review so that investigate suspected duplicate offline payment references and control exceptional override.
...
- **Runtime route:** `finance.payments-duplicates`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.payments-duplicates` (recorded in `docs/frontend/screens/finance/finance-duplicate-references.md`).
- **Data fields:** Assigned-branch duplicate-reference queue using masked normalized comparisons and server-owned risk facts, linking to the existing MFA and fresh-step-up override workflow without exposing raw references.
...
- **Permission-any:** — none
- **Permission-all:** `customer_payment.duplicate_override`
