...
>
> A real runtime route renders this page today: `finance.invoices` at `/finance/invoices` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/invoices`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `finance.invoices`
- **Navigation group:** Merchant-Client Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.invoices`
- **Screen key:** `invoices`
- **Authoritative map section:** §9.4.7

## Purpose

- **Purpose:** Review branch invoices and perform Finance-owned controlled void/adjustment workflows.
- **User story:** As merchant finance, I open Invoices so that review branch invoices and perform Finance-owned controlled void/adjustment workflows.
...
- **Runtime route:** `finance.invoices`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.invoices` (recorded in `docs/frontend/screens/finance/finance-invoices.md`).
- **Data fields:** Finance invoice list (branch-scoped, masked client): invoice number/'Draft', total, and status badge. Finance holds invoice.view but NOT invoice.create — no 'New invoice' button is shown.
