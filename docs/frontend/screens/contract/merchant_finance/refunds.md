# Screen specification — External Refunds

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `finance.refunds` at `/finance/refunds` (routes/finance.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/refunds`; owner phase **UI-12** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** External Refunds
- **Route:** `/refunds` (host-relative contract path)
- **Route name:** `finance.refunds`
- **Navigation group:** Merchant-Client Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.refunds`
- **Screen key:** `refunds`
- **Authoritative map section:** §9.4.12

## Purpose

- **Purpose:** Record and approve merchant-client refunds executed outside Servana.
- **User story:** As merchant finance, I open External Refunds so that record and approve merchant-client refunds executed outside Servana.
...
- **Runtime route:** `finance.refunds`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.refunds` (recorded in `docs/frontend/screens/finance/finance-refunds.md`).
- **Data fields:** Finance external-refund list with a status filter and a capability-gated 'Request refund' form (against a validated component; amount capped server-side at the remaining refundable; masked external reference). Servana never moves funds — a refund records the intent.
...
- **Icon:** `refund` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
