...
>
> A real runtime route renders this page today: `finance.dashboard` at `/dashboard` (routes/finance.ts), delivery **dedicated**.
...
- **Contract key:** `merchant_finance.dashboard`
- **Screen key:** `dashboard`
- **Authoritative map section:** §9.4.1
...
- **User story:** As merchant finance, I open Dashboard so that provide the Finance checker with a live branch-scoped financial-control home for validation, reconciliation, cash-up, period locks, payouts, liabilities, subscription payment, and high-risk tasks.
...
- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.dashboard` (recorded in `docs/frontend/screens/finance/finance-dashboard.md`).
- **Data fields:** Assigned-branch Finance command centre with server-derived payment, invoice, control, and compensation totals kept separate by currency; priority work links; explicit maker/checker and Wallet/report/notification gate truth; and no client-authored financial facts.
