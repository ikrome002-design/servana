# Screen specification — Partial and Split Payments

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `finance.payments-partial-split` at `/payments/partial-split` (routes/finance.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_finance
- **Host:** `finance.servana.ke`
- **Page title:** Partial and Split Payments
- **Route:** `/payments/partial-split` (host-relative contract path)
- **Route name:** `finance.payments-partial-split`
- **Navigation group:** Merchant-Client Finance
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_finance.payments-partial-split`
- **Screen key:** `payments-partial-split`
- **Authoritative map section:** §9.4.9

## Purpose

- **Purpose:** Review invoices with multiple payment groups or multi-method component allocations.
- **User story:** As merchant finance, I open Partial and Split Payments so that review invoices with multiple payment groups or multi-method component allocations.
...
- **Runtime route:** `finance.payments-partial-split`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `finance.payments-partial-split` (recorded in `docs/frontend/screens/finance/finance-partial-split-payments.md`).
- **Data fields:** Read-only assigned-branch invoice reconciliation surface showing server-derived totals, validated paid amount, balance, payment groups, masked components, duplicate state, and receipt outcomes without browser-authored money.
