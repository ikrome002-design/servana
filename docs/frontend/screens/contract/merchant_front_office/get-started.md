...
>
> A real runtime route renders this page today: `front-office.get-started` at `/front-office/get-started` (routes/frontOffice.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/get-started`; owner phase **UI-13** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `front-office.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §10.4.2

## Purpose

- **Purpose:** Guide Front Office through the complete client-to-receipt operational workflow.
- **User story:** As merchant front office, I open Get Started so that guide Front Office through the complete client-to-receipt operational workflow.
...
- **Runtime route:** `front-office.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.get-started` (recorded in `docs/frontend/screens/front-office/front-office-get-started.md`).
- **Data fields:** Guided get-started checklist for Front Office.
