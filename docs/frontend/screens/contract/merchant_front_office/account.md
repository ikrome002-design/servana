...
>
> A real runtime route renders this page today: `front-office.account` at `/account` (routes/frontOffice.ts), delivery **dedicated**.
...
- **Runtime route:** `front-office.account`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.account` (recorded in `docs/frontend/screens/front-office/front-office-account.md`).
- **Data fields:** Signed-in user's own Magic Link identity, MFA enrollment, active sessions, theme preference and assigned branch context; no business, staff-access or notification setting.
