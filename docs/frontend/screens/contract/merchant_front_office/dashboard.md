...
>
> A real runtime route renders this page today: `front-office.dashboard` at `/dashboard` (routes/frontOffice.ts), delivery **dedicated**.
...
- **Route name:** `front-office.dashboard`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.dashboard`
- **Screen key:** `dashboard`
- **Authoritative map section:** §10.4.1
...
- **User story:** As merchant front office, I open Dashboard so that provide a fast, client-facing branch operations home for arrivals, queue flow, service sessions, invoices, payment recording, receipt status, and simplified subscription recovery.
...
- **Runtime route:** `front-office.dashboard`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.dashboard` (recorded in `docs/frontend/screens/front-office/front-office-landing.md`).
- **Data fields:** Assigned-branch operational command centre for arrivals, queue, service, invoices, recorded payments and automatic receipt readiness.
