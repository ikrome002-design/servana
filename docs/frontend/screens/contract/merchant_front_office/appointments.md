...
>
> A real runtime route renders this page today: `front-office.appointments` at `/front-office/appointments` (routes/frontOffice.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/appointments`; owner phase **UI-13** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route:** `/appointments` (host-relative contract path)
- **Route name:** `front-office.appointments`
- **Navigation group:** Appointments & Walk-Ins
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.appointments`
- **Screen key:** `appointments`
- **Authoritative map section:** §10.4.7

## Purpose

- **Purpose:** Create, schedule, check in, reschedule, cancel, no-show, assign, and transfer branch appointments.
- **User story:** As merchant front office, I open Appointments so that create, schedule, check in, reschedule, cancel, no-show, assign, and transfer branch appointments.
...
- **Runtime route:** `front-office.appointments`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.appointments` (recorded in `docs/frontend/screens/front-office/front-office-appointments.md`).
- **Data fields:** Front Office appointment list for a chosen business date (branch-scoped): client, service, assigned personnel, status badge, start/end time, with date navigation and status filtering. Create button gated by appointment.create. Walk-ins/queue/sessions are later phases and are not shown here.
...
- **Permission-any:** — none
- **Permission-all:** `appointment.view`
