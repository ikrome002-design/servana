# Screen specification — Clients

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `front-office.clients` at `/front-office/clients` (routes/frontOffice.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/clients`; owner phase **UI-13** reconciles path shape (`UI01-ROUTE-003`).
...
- **Page title:** Clients
- **Route:** `/clients` (host-relative contract path)
- **Route name:** `front-office.clients`
- **Navigation group:** Clients
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.clients`
- **Screen key:** `clients`
- **Authoritative map section:** §10.4.4
...
- **User story:** As merchant front office, I open Clients so that search and manage branch client records used for appointments, walk-ins, sessions, invoices, and communication preferences.
...
- **Runtime route:** `front-office.clients`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.clients` (recorded in `docs/frontend/screens/front-office/front-office-clients.md`).
- **Data fields:** Front Office client directory + search by name or phone (branch/tenant-scoped). Contact is ALWAYS masked (phone last-four, masked email); the blind index is never shown.
...
- **Icon:** `client` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
