# Screen specification — Invoices

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `front-office.invoices` at `/front-office/invoices` (routes/frontOffice.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/invoices`; owner phase **UI-13** reconciles path shape (`UI01-ROUTE-003`).
...
- **Page title:** Invoices
- **Route:** `/invoices` (host-relative contract path)
- **Route name:** `front-office.invoices`
- **Navigation group:** Billing Client
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.invoices`
- **Screen key:** `invoices`
- **Authoritative map section:** §10.4.12

## Purpose

- **Purpose:** Create and view merchant-client invoices for completed or otherwise billable branch service activity.
- **User story:** As merchant front office, I open Invoices so that create and view merchant-client invoices for completed or otherwise billable branch service activity.
...
- **Runtime route:** `front-office.invoices`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.invoices` (recorded in `docs/frontend/screens/front-office/front-office-invoices.md`).
- **Data fields:** Front Office invoice list (branch-scoped): invoice number or 'Draft', masked client, server-formatted KES total, and status badge. A capability-gated 'New invoice' button (invoice.create) opens the draft builder. No payment or receipt control appears here (Phase 18).
