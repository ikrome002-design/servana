# Screen specification — Queue Transfer

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `front-office.queue-transfer` at `/queue/:queueUlid/transfer` (routes/frontOffice.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_front_office
- **Host:** `office.servana.ke`
- **Page title:** Queue Transfer
- **Route:** `/queue/:queueUlid/transfer` (host-relative contract path)
- **Route name:** `front-office.queue-transfer`
- **Navigation group:** Queue & Service › Queue
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.queue-transfer`
- **Screen key:** `queue-transfer`
- **Authoritative map section:** §10.4.10

## Purpose

- **Purpose:** Safely reassign a queue entry or associated appointment for operational continuity.
- **User story:** As merchant front office, I open Queue Transfer so that safely reassign a queue entry or associated appointment for operational continuity.
...
- **Runtime route:** `front-office.queue-transfer`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `front-office.queue-transfer` (recorded in `docs/frontend/screens/front-office/front-office-queue-transfer.md`).
- **Data fields:** Dedicated transfer screen retaining the queue subject beside a server-filtered eligible-personnel form; reason, branch scope and queue state remain backend-enforced.
...
- **Icon:** `transfer` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
