...
>
> A real runtime route renders this page today: `front-office.sessions` at `/front-office/sessions` (routes/frontOffice.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/sessions`; owner phase **UI-13** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_front_office
- **Host:** `office.servana.ke`
- **Page title:** Service Sessions
- **Route:** `/sessions` (host-relative contract path)
- **Route name:** `front-office.sessions`
- **Navigation group:** Queue & Service
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_front_office.sessions`
- **Screen key:** `sessions`
- **Authoritative map section:** §10.4.11

## Purpose

- **Purpose:** Create and manage the operational service-delivery record linked to queue/appointment and later invoicing.
- **User story:** As merchant front office, I open Service Sessions so that create and manage the operational service-delivery record linked to queue/appointment and later invoicing.
...
- **Permission-any:** — none
- **Permission-all:** `service_session.view`
...
- **Icon:** `session` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
