# Screen specification — Cash-Up Submission

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `branch.cash-up` at `/branch/cash-up` (routes/branch.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/cash-up`; owner phase **UI-10** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_branch
- **Host:** `branch.servana.ke`
- **Page title:** Cash-Up Submission
- **Route:** `/cash-up` (host-relative contract path)
- **Route name:** `branch.cash-up`
- **Navigation group:** Branch Operations
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.cash-up`
- **Screen key:** `cash-up`
- **Authoritative map section:** §7.4.13

## Purpose

- **Purpose:** Let the Branch Manager prepare and submit daily method-level reconciliation to Finance.
- **User story:** As merchant branch, I open Cash-Up Submission so that let the Branch Manager prepare and submit daily method-level reconciliation to Finance.
...
- **Runtime route:** `branch.cash-up`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.cash-up` (recorded in `docs/frontend/screens/branch/branch-cash-up.md`).
- **Data fields:** Branch Manager cash-up (maker): a business-date branch-day cash-up with one line per concrete payment method. Expected amounts are SERVER-derived and never editable; the Branch Manager enters counted amounts, sees per-method + total variance, saves a draft, and submits for Finance review (or resubmits after a correction request). Submitted / approved / locked cash-ups are read-only. The Branch Manager can NEVER approve — approval is Finance, backend-enforced. No-branch boundary shown when unassigned.
...
- **Permission-any:** — none
- **Permission-all:** `branch.cash_up.submit`
...
- **Icon:** `cash-up` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
