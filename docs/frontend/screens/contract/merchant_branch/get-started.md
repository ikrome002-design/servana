...
>
> A real runtime route renders this page today: `branch.get-started` at `/branch/get-started` (routes/branch.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/get-started`; owner phase **UI-10** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `branch.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §7.4.2

## Purpose

- **Purpose:** Guide the Branch Manager through branch readiness in the correct order.
- **User story:** As merchant branch, I open Get Started so that guide the Branch Manager through branch readiness in the correct order.
...
- **Runtime route:** `branch.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.get-started` (recorded in `docs/frontend/screens/branch/branch-get-started.md`).
- **Data fields:** Guided get-started checklist for branch setup.
