...
>
> A real runtime route renders this page today: `branch.audit` at `/audit` (routes/branch.ts), delivery **dedicated**.
...
- **Route name:** `branch.audit`
- **Navigation group:** Reporting
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.audit`
- **Screen key:** `audit`
- **Authoritative map section:** §7.4.15

## Purpose

- **Purpose:** Provide Branch Manager read visibility into relevant branch changes without replacing the Audit account.
- **User story:** As merchant branch, I open Branch Audit Log so that provide Branch Manager read visibility into relevant branch changes without replacing the Audit account.
...
- **Runtime route:** `branch.audit`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.audit` (recorded in `docs/frontend/screens/branch/branch-audit-log.md`).
- **Data fields:** Paginated read-only branch audit-event projection that complements, but does not replace, the dedicated Audit account.
