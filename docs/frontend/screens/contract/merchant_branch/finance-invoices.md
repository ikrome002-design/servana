...
>
> A real runtime route renders this page today: `branch.finance-invoices` at `/finance/invoices` (routes/branch.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_branch
- **Host:** `branch.servana.ke`
- **Page title:** Invoices Read View
- **Route:** `/finance/invoices` (host-relative contract path)
- **Route name:** `branch.finance-invoices`
- **Navigation group:** Financial Visibility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.finance-invoices`
- **Screen key:** `finance-invoices`
- **Authoritative map section:** §7.4.10

## Purpose

- **Purpose:** Provide branch-level invoice status and revenue context without invoice-creation or finance-mutation authority.
- **User story:** As merchant branch, I open Invoices Read View so that provide branch-level invoice status and revenue context without invoice-creation or finance-mutation authority.

## Ownership and status

- **UI owner phase:** **UI-10**
- **Backend owner phase:** **Phase UI-10 (COR-UI10-001)**
- **Implementation status:** `implemented`
- **Runtime route:** `branch.finance-invoices`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.finance-invoices` (recorded in `docs/frontend/screens/branch/branch-invoices-read-view.md`).
- **Data fields:** Paginated branch-scoped invoice status and revenue visibility with every invoice mutation capability explicitly absent.
