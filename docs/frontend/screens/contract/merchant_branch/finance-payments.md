# Screen specification — Payment Records Read View

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `branch.finance-payments` at `/finance/payments` (routes/branch.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_branch
- **Host:** `branch.servana.ke`
- **Page title:** Payment Records Read View
- **Route:** `/finance/payments` (host-relative contract path)
- **Route name:** `branch.finance-payments`
- **Navigation group:** Financial Visibility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.finance-payments`
- **Screen key:** `finance-payments`
- **Authoritative map section:** §7.4.11

## Purpose

- **Purpose:** Show recorded merchant-client payment groups and validation states for branch operational awareness.
- **User story:** As merchant branch, I open Payment Records Read View so that show recorded merchant-client payment groups and validation states for branch operational awareness.
...
- **Runtime route:** `branch.finance-payments`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.finance-payments` (recorded in `docs/frontend/screens/branch/branch-payments-read-view.md`).
- **Data fields:** Paginated branch-scoped payment-record status visibility without recording, validation, rejection or reference-correction authority.
