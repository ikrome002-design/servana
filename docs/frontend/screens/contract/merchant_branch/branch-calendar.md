# Screen specification — Operating Calendar

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `branch.branch-calendar` at `/branch/calendar` (routes/branch.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_branch
- **Host:** `branch.servana.ke`
- **Page title:** Operating Calendar
- **Route:** `/branch/calendar` (host-relative contract path)
- **Route name:** `branch.branch-calendar`
- **Navigation group:** Branch Setup
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.branch-calendar`
- **Screen key:** `branch-calendar`
- **Authoritative map section:** §7.4.4

## Purpose

- **Purpose:** Define when the branch can accept appointments, walk-ins, queues, and service sessions.
- **User story:** As merchant branch, I open Operating Calendar so that define when the branch can accept appointments, walk-ins, queues, and service sessions.
...
- **Runtime route:** `branch.branch-calendar`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.branch-calendar` (recorded in `docs/frontend/screens/branch/branch-operating-calendar.md`).
- **Data fields:** Canonical Branch account route for weekly operating hours, split sessions and date-specific branch calendar exceptions.
...
- **Permission-any:** — none
- **Permission-all:** `branch.calendar.manage`
...
- **Icon:** `calendar` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
