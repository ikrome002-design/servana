...
>
> A real runtime route renders this page today: `branch.branch-profile` at `/branch/profile` (routes/branch.ts), delivery **dedicated**.
...
- **Runtime route:** `branch.branch-profile`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.branch-profile` (recorded in `docs/frontend/screens/branch/branch-profile.md`).
- **Data fields:** Assigned-branch identity, contact, address, category and status context with branch-scoped profile management authority.
...
- **Permission-any:** — none
- **Permission-all:** `branch.profile.manage`
