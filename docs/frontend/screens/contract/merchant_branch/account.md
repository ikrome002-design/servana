...
>
> A real runtime route renders this page today: `branch.account` at `/account` (routes/branch.ts), delivery **dedicated**.
...
- **Route name:** `branch.account`
- **Navigation group:** Utility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.account`
- **Screen key:** `account`
- **Authoritative map section:** §7.4.18

## Purpose

- **Purpose:** Manage the Branch Manager's own profile, sessions, preferences, and branch context.
- **User story:** As merchant branch, I open Account and Preferences so that manage the Branch Manager's own profile, sessions, preferences, and branch context.

## Ownership and status

- **UI owner phase:** **UI-10**
- **Backend owner phase:** **Phase UI-04**
- **Implementation status:** `implemented`
- **Runtime route:** `branch.account`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.account` (recorded in `docs/frontend/screens/branch/branch-account-security.md`).
- **Data fields:** Own-scope identity, verified email, MFA, Magic Link sessions, theme preference and account-context switching with Branch wording and no passwords.
