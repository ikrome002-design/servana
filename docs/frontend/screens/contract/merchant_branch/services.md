# Screen specification — Service Catalogue

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `branch.services` at `/branch/services` (routes/branch.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/services`; owner phase **UI-10** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_branch
- **Host:** `branch.servana.ke`
- **Page title:** Service Catalogue
- **Route:** `/services` (host-relative contract path)
- **Route name:** `branch.services`
- **Navigation group:** Branch Operations
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_branch.services`
- **Screen key:** `services`
- **Authoritative map section:** §7.4.6

## Purpose

- **Purpose:** Create, price, schedule, and archive the branch's services.
- **User story:** As merchant branch, I open Service Catalogue so that create, price, schedule, and archive the branch's services.
...
- **Runtime route:** `branch.services`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `branch.services` (recorded in `docs/frontend/screens/branch/service-catalogue.md`).
- **Data fields:** Branch Manager service catalogue: list/filter, create/edit (price in minor units, duration, category), archive with confirmation, and a read-only eligibility view. Contact-free; the legacy preferred-personnel fee is never shown or editable.
...
- **Icon:** `service` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
