...
>
> Blocked by **external_gate_w**. The navigation entry is rendered disabled and names the gate; it has no live destination, and no Wallet, Refer & Earn, notification or provider runtime exists behind it. Owner phase **UI-15** implements the page once the gate opens.
...
- **Route name:** `audit.notifications`
- **Navigation group:** Utility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_audit.notifications`
- **Screen key:** `notifications`
- **Authoritative map section:** §12.4.14

## Purpose

- **Purpose:** Show high-risk event, flagged review, export, integrity, and access-security alerts.
- **User story:** As merchant audit, I open Notifications so that show high-risk event, flagged review, export, integrity, and access-security alerts.
