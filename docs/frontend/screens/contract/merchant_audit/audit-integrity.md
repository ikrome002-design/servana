# Screen specification — Integrity Status
...
- **Page title:** Integrity Status
- **Route:** `/audit/integrity` (host-relative contract path)
- **Route name:** `audit.audit-integrity`
- **Navigation group:** Audit
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_audit.audit-integrity`
- **Screen key:** `audit-integrity`
- **Authoritative map section:** §12.4.13

## Purpose

- **Purpose:** Display audit-chain verification status and incidents relevant to the assigned branch.
- **User story:** As merchant audit, I open Integrity Status so that display audit-chain verification status and incidents relevant to the assigned branch.
...
- **Icon:** `integrity` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
