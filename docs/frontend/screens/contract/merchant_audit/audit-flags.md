# Screen specification — Flagged Events

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `audit.flagged-events` at `/audit/flagged` (routes/audit.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/audit/flags`; owner phase **UI-15** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_audit
- **Host:** `audit.servana.ke`
- **Page title:** Flagged Events
- **Route:** `/audit/flags` (host-relative contract path)
- **Route name:** `audit.audit-flags`
- **Navigation group:** Review
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_audit.audit-flags`
- **Screen key:** `audit-flags`
- **Authoritative map section:** §12.4.5

## Purpose

- **Purpose:** Manage the only Audit-role mutation: review metadata attached to suspicious audit events.
- **User story:** As merchant audit, I open Flagged Events so that manage the only Audit-role mutation: review metadata attached to suspicious audit events.
...
- **Runtime route:** `audit.flagged-events`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `audit.flagged-events` (recorded in `docs/frontend/screens/audit/audit-flagged-queue.md`).
- **Data fields:** Branch-scoped flagged-event queue across open/under_review/resolved/dismissed/reopened with status filter, assignee and review-status summaries, and timestamps.
...
- **Icon:** `flag` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
