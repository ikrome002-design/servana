...
- **Page title:** My Queue
- **Route:** `/work/queue` (host-relative contract path)
- **Route name:** `personnel.work-queue`
- **Navigation group:** My Work
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.work-queue`
- **Screen key:** `work-queue`
- **Authoritative map section:** §11.4.3

## Purpose

- **Purpose:** Show only queue entries assigned to the logged-in Personnel member.
- **User story:** As merchant personnel, I open My Queue so that show only queue entries assigned to the logged-in Personnel member.
...
- **Permission-any:** — none
- **Permission-all:** `personnel.my_queue.view`
