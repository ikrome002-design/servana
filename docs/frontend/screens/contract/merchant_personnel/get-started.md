...
>
> A real runtime route renders this page today: `personnel.get-started` at `/personnel/get-started` (routes/personnel.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/get-started`; owner phase **UI-14** reconciles path shape (`UI01-ROUTE-003`).
...
- **Route name:** `personnel.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §11.4.2

## Purpose

- **Purpose:** Help Personnel understand their own work, compensation terms, client access, and permitted messaging.
- **User story:** As merchant personnel, I open Get Started so that help Personnel understand their own work, compensation terms, client access, and permitted messaging.
...
- **Runtime route:** `personnel.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `personnel.get-started` (recorded in `docs/frontend/screens/personnel/personnel-get-started.md`).
- **Data fields:** Guided get-started checklist for Personnel.
