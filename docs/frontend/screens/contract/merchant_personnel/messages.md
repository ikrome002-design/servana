# Screen specification — Message History
...
- **Route:** `/messages` (host-relative contract path)
- **Route name:** `personnel.messages`
- **Navigation group:** My Clients
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.messages`
- **Screen key:** `messages`
- **Authoritative map section:** §11.4.10

## Purpose

- **Purpose:** Show the logged-in Personnel member's own in-platform SMS sends and outcomes.
- **User story:** As merchant personnel, I open Message History so that show the logged-in Personnel member's own in-platform SMS sends and outcomes.
