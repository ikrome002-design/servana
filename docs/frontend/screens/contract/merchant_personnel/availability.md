# Screen specification — Availability Status
...
- **Page title:** Availability Status
- **Route:** `/availability` (host-relative contract path)
- **Route name:** `personnel.availability`
- **Navigation group:** My Work
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.availability`
- **Screen key:** `availability`
- **Authoritative map section:** §11.4.18

## Purpose

- **Purpose:** Show HR-controlled schedule and permit only limited live status toggles allowed by policy.
- **User story:** As merchant personnel, I open Availability Status so that show HR-controlled schedule and permit only limited live status toggles allowed by policy.
