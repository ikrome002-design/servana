...
- **Page title:** Salary
- **Route:** `/earnings/salary` (host-relative contract path)
- **Route name:** `personnel.earnings-salary`
- **Navigation group:** My Earnings
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.earnings-salary`
- **Screen key:** `earnings-salary`
- **Authoritative map section:** §11.4.13
...
- **User story:** As merchant personnel, I open Salary so that show salary terms, accrual, due, approved, paid, and adjusted amounts for salary-only or salary-plus-commission Personnel.
...
- **Icon:** `salary` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
