...
>
> Blocked by **external_gate_w**. The navigation entry is rendered disabled and names the gate; it has no live destination, and no Wallet, Refer & Earn, notification or provider runtime exists behind it. Owner phase **UI-14** implements the page once the gate opens.
...
- **Route name:** `personnel.notifications`
- **Navigation group:** Utility
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.notifications`
- **Screen key:** `notifications`
- **Authoritative map section:** §11.4.19

## Purpose

- **Purpose:** Show own work, preferred-request, SMS, compensation, payout, and query notices.
- **User story:** As merchant personnel, I open Notifications so that show own work, preferred-request, SMS, compensation, payout, and query notices.
