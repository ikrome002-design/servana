# Screen specification — SMS Composer
...
- **Page title:** SMS Composer
- **Route:** `/messages/compose` (host-relative contract path)
- **Route name:** `personnel.messages-compose`
- **Navigation group:** My Clients
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_personnel.messages-compose`
- **Screen key:** `messages-compose`
- **Authoritative map section:** §11.4.9

## Purpose

- **Purpose:** Send in-platform SMS to all or selected personally served clients without exposing raw contact data.
- **User story:** As merchant personnel, I open SMS Composer so that send in-platform SMS to all or selected personally served clients without exposing raw contact data.
...
- **Permission-any:** — none
- **Permission-all:** `personnel.my_sms.send`
