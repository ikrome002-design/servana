# Screen specification — Billing Reconciliation Exceptions
...
- **Page title:** Billing Reconciliation Exceptions
- **Route:** `/billing/reconciliation-exceptions` (host-relative contract path)
- **Route name:** `platform.billing-reconciliation-exceptions`
- **Navigation group:** Billing Operations
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.billing-reconciliation-exceptions`
- **Screen key:** `billing-reconciliation-exceptions`
- **Authoritative map section:** §5.4.14
...
- **User story:** As super administrator, I open Billing Reconciliation Exceptions so that resolve mismatches between Wallet-confirmed money movement and Servana billing allocation without creating a manual payment path.
...
- **Icon:** `reconciliation` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
