...
>
> A real runtime route renders this page today: `platform.billing-preferred-personnel-fees` at `/billing/preferred-personnel-fees` (routes/platform.ts), delivery **dedicated**.
...
- **Runtime route:** `platform.billing-preferred-personnel-fees`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `platform.billing-preferred-personnel-fees` (recorded in `docs/frontend/screens/platform/platform-billing-preferred-personnel-fees.md`).
- **Data fields:** Contract page §5.4.8. Fixed or percentage preferred-personnel fee rules at platform-default or per-service scope. Rules supersede rather than edit, so a rule that priced a past booking stays intact; approve and cancel require MFA and a fresh step-up.
