# Screen specification — Internal Platform Access

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `platform.platform-access` at `/platform-access` (routes/platform.ts), delivery **dedicated**.

## Identity

- **Account:** super_administrator
- **Host:** `citrus.servana.ke`
- **Page title:** Internal Platform Access
- **Route:** `/platform-access` (host-relative contract path)
- **Route name:** `platform.platform-access`
- **Navigation group:** Platform Administration
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.platform-access`
- **Screen key:** `platform-access`
- **Authoritative map section:** §5.4.19

## Purpose

- **Purpose:** Manage Citrus Labs internal platform users, roles, permissions, MFA status, and access lifecycle.
- **User story:** As super administrator, I open Internal Platform Access so that manage Citrus Labs internal platform users, roles, permissions, MFA status, and access lifecycle.
...
- **Runtime route:** `platform.platform-access`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `platform.platform-access` (recorded in `docs/frontend/screens/platform/platform-platform-access.md`).
- **Data fields:** Contract page §5.4.19 (COR-UI08-001). Invitation, permission and lifecycle management for internal Citrus platform users only. A platform user can never be given a merchant role, merchant membership, branch assignment or staff profile. Self-escalation is refused, the last active administrator cannot be removed or suspended, and every sensitive action requires MFA and a fresh step-up.
...
- **Permission-any:** — none
- **Permission-all:** `platform.internal_access.view`
