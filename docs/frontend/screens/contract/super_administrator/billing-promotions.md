# Screen specification — Promotional Discounts
...
- **Page title:** Promotional Discounts
- **Route:** `/billing/promotions` (host-relative contract path)
- **Route name:** `platform.billing-promotions`
- **Navigation group:** Billing & Commercial
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.billing-promotions`
- **Screen key:** `billing-promotions`
- **Authoritative map section:** §5.4.6

## Purpose

- **Purpose:** Configure percentage or fixed-amount promotions with deterministic targeting and immutable application snapshots.
- **User story:** As super administrator, I open Promotional Discounts so that configure percentage or fixed-amount promotions with deterministic targeting and immutable application snapshots.
...
- **Icon:** `promotion` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
