# Screen specification — Plan Prices and Billing Periods

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `platform.billing-prices` at `/billing/prices` (routes/platform.ts), delivery **dedicated**.

## Identity

- **Account:** super_administrator
- **Host:** `citrus.servana.ke`
- **Page title:** Plan Prices and Billing Periods
- **Route:** `/billing/prices` (host-relative contract path)
- **Route name:** `platform.billing-prices`
- **Navigation group:** Billing & Commercial
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.billing-prices`
- **Screen key:** `billing-prices`
- **Authoritative map section:** §5.4.5
...
- **User story:** As super administrator, I open Plan Prices and Billing Periods so that maintain the sole source of truth for effective-dated subscription prices and the five canonical billing intervals.
...
- **Runtime route:** `platform.billing-prices`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `platform.billing-prices` (recorded in `docs/frontend/screens/platform/platform-billing-prices.md`).
- **Data fields:** Contract page §5.4.5. Effective-dated plan prices across the five billing intervals. Overlapping effective ranges are rejected server-side, only future prices are cancellable, and current and historical prices are read-only so an issued invoice can never be rewritten.
...
- **Icon:** `price` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
