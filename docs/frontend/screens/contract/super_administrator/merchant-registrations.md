# Screen specification — Registration Monitoring
...
- **Page title:** Registration Monitoring
- **Route:** `/merchants/registrations` (host-relative contract path)
- **Route name:** `platform.merchant-registrations`
- **Navigation group:** Merchants
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.merchant-registrations`
- **Screen key:** `merchant-registrations`
- **Authoritative map section:** §5.4.10

## Purpose

- **Purpose:** Monitor self-registration quality and abuse without introducing manual merchant activation.
- **User story:** As super administrator, I open Registration Monitoring so that monitor self-registration quality and abuse without introducing manual merchant activation.
...
- **Permission-any:** — none
- **Permission-all:** `platform.registration_monitor.view`
...
- **Icon:** `registration` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
