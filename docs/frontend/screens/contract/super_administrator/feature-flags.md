# Screen specification — Feature Flags

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `platform.feature-flags` at `/platform/feature-flags` (routes/platform.ts), delivery **dedicated**.
...
- **Route:** `/platform/feature-flags` (host-relative contract path)
- **Route name:** `platform.feature-flags`
- **Navigation group:** Platform Administration
- **Navigation placement:** header primary navigation
- **Contract key:** `super_administrator.feature-flags`
- **Screen key:** `feature-flags`
- **Authoritative map section:** §5.4.20

## Purpose

- **Purpose:** Control approved platform feature rollout without bypassing permissions, entitlements, or billing rules.
- **User story:** As super administrator, I open Feature Flags so that control approved platform feature rollout without bypassing permissions, entitlements, or billing rules.
...
- **Runtime route:** `platform.feature-flags`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `platform.feature-flags` (recorded in `docs/frontend/screens/platform/platform-feature-flags.md`).
- **Data fields:** Contract page §5.4.20 (COR-UI08-001). Maker and checker feature-flag governance with an append-only history. A flag can never grant a permission, bypass an entitlement, bypass a billing state or open an external gate: it only enables or disables an already-authorized capability.
...
- **Permission-any:** — none
- **Permission-all:** `platform.settings.view`
...
- **Icon:** `feature-flag` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
