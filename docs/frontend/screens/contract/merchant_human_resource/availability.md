...
>
> A real runtime route renders this page today: `hr.availability` at `/hr/availability` (routes/hr.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/availability`; owner phase **UI-11** reconciles path shape (`UI01-ROUTE-003`).
