...
>
> A real runtime route renders this page today: `hr.compensation-setup` at `/compensation/:staffUlid/setup` (routes/hr.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_human_resource
- **Host:** `hr.servana.ke`
- **Page title:** Compensation Setup
- **Route:** `/compensation/:staffUlid/setup` (host-relative contract path)
- **Route name:** `hr.compensation-setup`
- **Navigation group:** Compensation › Compensation Detail
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_human_resource.compensation-setup`
- **Screen key:** `compensation-setup`
- **Authoritative map section:** §8.4.13

## Purpose

- **Purpose:** Create an effective-dated commission-only, salary-plus-commission, or salary-only compensation plan.
- **User story:** As merchant human resource, I open Compensation Setup so that create an effective-dated commission-only, salary-plus-commission, or salary-only compensation plan.
...
- **Runtime route:** `hr.compensation-setup`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `hr.compensation-setup` (recorded in `docs/frontend/screens/hr/hr-compensation-setup.md`).
- **Data fields:** Create an effective-dated compensation plan for one staff member using the shipped configuration workflow.
...
- **Permission-any:** — none
- **Permission-all:** `compensation.plan.create`
