...
>
> A real runtime route renders this page today: `hr.compensation-detail` at `/compensation/:staffUlid` (routes/hr.ts), delivery **dedicated**.
...
- **Runtime route:** `hr.compensation-detail`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `hr.compensation-detail` (recorded in `docs/frontend/screens/hr/hr-compensation-detail.md`).
- **Data fields:** Staff-addressed current, scheduled, and historical compensation configuration without earnings data.
