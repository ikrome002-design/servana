...
>
> A real runtime route renders this page today: `hr.staff-detail-lifecycle` at `/staff/:staffUlid/lifecycle` (routes/hr.ts), delivery **dedicated**.

## Identity

- **Account:** merchant_human_resource
- **Host:** `hr.servana.ke`
- **Page title:** Staff Lifecycle
- **Route:** `/staff/:staffUlid/lifecycle` (host-relative contract path)
- **Route name:** `hr.staff-detail-lifecycle`
- **Navigation group:** Staff › Staff Detail
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_human_resource.staff-detail-lifecycle`
- **Screen key:** `staff-detail-lifecycle`
- **Authoritative map section:** §8.4.7
...
- **User story:** As merchant human resource, I open Staff Lifecycle so that control invitation, activation, suspension, reactivation, and deactivation with immediate access revocation and historical preservation.

## Ownership and status

- **UI owner phase:** **UI-11**
- **Backend owner phase:** **Phase 7 (UI-11 canonical composition)**
- **Implementation status:** `implemented`
- **Runtime route:** `hr.staff-detail-lifecycle`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `hr.staff-detail-lifecycle` (recorded in `docs/frontend/screens/hr/hr-staff-lifecycle.md`).
- **Data fields:** Branch-scoped staff suspension, reactivation, and deactivation with explicit confirmation.
