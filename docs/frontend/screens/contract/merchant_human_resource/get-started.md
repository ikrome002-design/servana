...
- **Route name:** `hr.get-started`
- **Navigation group:** Home
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_human_resource.get-started`
- **Screen key:** `get-started`
- **Authoritative map section:** §8.4.2

## Purpose

- **Purpose:** Guide HR through staff activation, service readiness, and compensation setup.
- **User story:** As merchant human resource, I open Get Started so that guide HR through staff activation, service readiness, and compensation setup.
...
- **Runtime route:** `hr.get-started`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `hr.get-started` (recorded in `docs/frontend/screens/hr/hr-get-started.md`).
- **Data fields:** Guided get-started checklist for HR setup.
