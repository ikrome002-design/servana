# Screen specification — Service Eligibility

> GENERATED FILE — do not edit.
> Source: `docs/frontend/navigation/servana-user-account-navigation-map.yaml` · Regenerate: `node scripts/generate-ui07-navigation-contract.mjs`
>
> A real runtime route renders this page today: `hr.eligibility` at `/hr/eligibility` (routes/hr.ts), delivery **dedicated**. The runtime path uses the account's path prefix rather than the host-relative contract path `/eligibility`; owner phase **UI-11** reconciles path shape (`UI01-ROUTE-003`).

## Identity

- **Account:** merchant_human_resource
- **Host:** `hr.servana.ke`
- **Page title:** Service Eligibility
- **Route:** `/eligibility` (host-relative contract path)
- **Route name:** `hr.eligibility`
- **Navigation group:** Workforce Readiness
- **Navigation placement:** sidebar primary navigation
- **Contract key:** `merchant_human_resource.eligibility`
- **Screen key:** `eligibility`
- **Authoritative map section:** §8.4.9

## Purpose

- **Purpose:** Define which Personnel can perform which branch services.
- **User story:** As merchant human resource, I open Service Eligibility so that define which Personnel can perform which branch services.
...
- **Runtime route:** `hr.eligibility`
- **Route delivery:** `dedicated`
- **External gate:** none

## Data and behaviour

- **API dependencies:** `GET /api/v1/me` bootstrap plus the endpoints already backing `hr.eligibility` (recorded in `docs/frontend/screens/hr/service-eligibility.md`).
- **Data fields:** HR manages which personnel are eligible for each service within HR's assigned branch: select a service, assign/revoke eligible personnel (same-branch only).
...
- **Permission-any:** — none
- **Permission-all:** `personnel.eligibility.manage`
...
- **Icon:** `eligibility` (Heroicons, resolved through the curated navigation icon registry — no emoji, no runtime arbitrary lookup).
