...
- **Screen key:** `audit-event-list`
- **Route name and URL:** `audit.branch-events`
- **Layout:** `AuditLayout`
- **Allowed roles:** `merchant_audit`
- **Required permissions:** `audit.branch_events.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Read-only, branch-scoped, field-masked audit event list with allowlisted severity/action/date filters, sort, and pagination; merchant-level (branch_id null) rows are never shown; no raw before/after/context.
