...
- **Screen key:** `audit-platform-fees`
- **Route name and URL:** `audit.platform-fees`
- **Layout:** `AuditLayout`
- **Allowed roles:** `merchant_audit`
- **Required permissions:** `platform_fee.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Masked, branch-scoped, read-only platform-fee visibility for Audit. No dispute-create or review controls, and no mutation anywhere. No private client contact, raw payment references, internal ids, or evidence content is shown. No Wallet/settlement UI.
