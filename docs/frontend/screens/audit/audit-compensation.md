...
- **Route name and URL:** `audit.compensation`
- **Layout:** `AuditLayout`
- **Allowed roles:** `merchant_audit`
- **Required permissions:** `audit.compensation.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Read-only, branch-scoped, masked compensation-domain audit view; honest empty state until the owning phases (20F-20H) emit events; no compensation events/routes/data fabricated.
