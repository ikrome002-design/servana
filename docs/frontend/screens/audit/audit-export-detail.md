# Screen specification — Audit export detail
...
- **Screen key:** `audit-export-detail`
- **Route name and URL:** `audit.export-detail`
- **Layout:** `AuditLayout`
- **Allowed roles:** `merchant_audit`
- **Required permissions:** `audit.export` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Export detail with queued/processing/ready/failed/expired/revoked polling, private on-demand signed download (never stored; accounting changes after stream), and authorized revoke with confirmation.
