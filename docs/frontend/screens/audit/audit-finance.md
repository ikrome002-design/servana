...
- **Route name and URL:** `audit.finance`
- **Layout:** `AuditLayout`
- **Allowed roles:** `merchant_audit`
- **Required permissions:** `audit.finance.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Read-only, branch-scoped, masked finance-domain audit view (Audit via audit.finance.view; Finance via finance.audit.view with server MFA); not a Finance operational screen.
