...
- **Screen key:** `hr-compensation-history`
- **Route name and URL:** `hr.compensation-history`
- **Layout:** `HumanResourceLayout`
- **Allowed roles:** `merchant_human_resource`
- **Required permissions:** `compensation.history.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Append-only compensation plan event history for one branch-scoped staff member.
