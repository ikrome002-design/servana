...
- **Screen key:** `service-eligibility`
- **Route name and URL:** `hr.eligibility`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_human_resource`
- **Required permissions:** `personnel.eligibility.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** HR manages which personnel are eligible for each service within HR's assigned branch: select a service, assign/revoke eligible personnel (same-branch only).
