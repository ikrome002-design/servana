# Screen specification — Availability
...
- **Screen key:** `personnel-availability`
- **Route name and URL:** `hr.availability`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_human_resource`
- **Required permissions:** `personnel.availability.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
