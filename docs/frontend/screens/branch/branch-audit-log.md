...
- **Screen key:** `branch-audit-log`
- **Route name and URL:** `branch.audit`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_branch`
- **Required permissions:** `reports.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Paginated read-only branch audit-event projection that complements, but does not replace, the dedicated Audit account.
