...
- **Route name and URL:** `branch.branch-profile`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_branch`
- **Required permissions:** `branch.profile.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Assigned-branch identity, contact, address, category and status context with branch-scoped profile management authority.
