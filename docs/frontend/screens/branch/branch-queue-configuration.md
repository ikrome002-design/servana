# Screen specification — Queue settings
...
- **Screen key:** `branch-queue-configuration`
- **Route name and URL:** `branch.queue-configuration`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_branch`
- **Required permissions:** `branch.profile.manage`, `day.open_close` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Branch Manager queue configuration on today's Branch Day: open/close the queue, set capacity (cannot be below the current active count), and set the default assignment mode (next available or manual). Not an operational entry mutation; the backend enforces branch.profile.manage + day.open_close.
