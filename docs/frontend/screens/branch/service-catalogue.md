...
- **Screen key:** `service-catalogue`
- **Route name and URL:** `branch.services`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_branch`
- **Required permissions:** `service.view`, `service.create`, `service.update`, `service.archive` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Branch Manager service catalogue: list/filter, create/edit (price in minor units, duration, category), archive with confirmation, and a read-only eligibility view. Contact-free; the legacy preferred-personnel fee is never shown or editable.
