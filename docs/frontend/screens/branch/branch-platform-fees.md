...
- **Screen key:** `branch-platform-fees`
- **Route name and URL:** `branch.platform-fees`
- **Layout:** `BranchLayout`
...
- **Fields and displayed data:** Branch-attributable, read-only platform-fee visibility. The backend server-scopes entries to the actor assigned branches; no merchant-wide aggregate, no dispute or configuration controls. No Wallet/settlement UI.
