...
- **Screen key:** `branch-day-operations`
- **Route name and URL:** `branch.branch-day`
- **Layout:** `BranchLayout`
- **Allowed roles:** `merchant_branch`
- **Required permissions:** `day.open_close` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Server-authoritative branch business-day state machine with explicit opening, closing and unresolved close-blocker treatment.
