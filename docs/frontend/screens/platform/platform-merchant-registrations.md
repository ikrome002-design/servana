...
- **Screen key:** `platform-merchant-registrations`
- **Route name and URL:** `platform.merchant-registrations`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.registration_monitor.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
