...
- **Screen key:** `platform-billing-sms`
- **Route name and URL:** `platform.billing-sms`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.billing_settings.view`, `platform.billing_settings.update` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.9 (COR-UI08-001). Effective-dated platform SMS billing rules, per-merchant usage and charge reconciliation. A rule that has priced a segment is immutable: a change is a scheduled successor, and a scheduled rule may be cancelled before it takes effect. The page shows no recipient, no message body and no contact list, because SMS recipient export does not exist anywhere in Servana.
