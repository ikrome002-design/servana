...
- **Screen key:** `platform-billing-prices`
- **Route name and URL:** `platform.billing-prices`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.plan.view`, `platform.plan_price.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.5. Effective-dated plan prices across the five billing intervals. Overlapping effective ranges are rejected server-side, only future prices are cancellable, and current and historical prices are read-only so an issued invoice can never be rewritten.
