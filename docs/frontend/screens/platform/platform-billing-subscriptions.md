...
- **Screen key:** `platform-billing-subscriptions`
- **Route name and URL:** `platform.billing-subscriptions`
...
- **Fields and displayed data:** Contract page §5.4.13 (COR-UI08-001). Platform-wide monitoring of subscriptions, subscription invoices, billing credits and escalations over seven READ projections. There is no mutation of any kind: no record-payment, mark-paid, edit-invoice, override-state, create-credit or query-provider control exists, because a subscription state is changed by the billing lifecycle and an issued invoice is immutable.
