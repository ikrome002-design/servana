...
- **Screen key:** `platform-billing-preferred-personnel-fees`
- **Route name and URL:** `platform.billing-preferred-personnel-fees`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.preferred_personnel_fee.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.8. Fixed or percentage preferred-personnel fee rules at platform-default or per-service scope. Rules supersede rather than edit, so a rule that priced a past booking stays intact; approve and cancel require MFA and a fresh step-up.
