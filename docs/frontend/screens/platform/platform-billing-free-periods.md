...
- **Screen key:** `platform-billing-free-periods`
- **Route name and URL:** `platform.billing-free-periods`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.free_period_offer.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.7. Free-period offers with the same deterministic targeting, lifecycle and immutability rules as promotional discounts, rendered through the same shared Phase 20C form. Draft, approve, pause, resume and cancel each require a mandatory reason, MFA and a fresh server-enforced step-up.
