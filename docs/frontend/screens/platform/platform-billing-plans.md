...
- **Screen key:** `platform-billing-plans`
- **Route name and URL:** `platform.billing-plans`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.plan.view`, `platform.plan.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.4. Subscription plan metadata and the SERVER-enforced entitlements attached to each plan. Selecting a plan reveals its entitlements on the same page. Retiring a plan preserves existing subscription history, and nothing selected here creates a frontend-only restriction.
