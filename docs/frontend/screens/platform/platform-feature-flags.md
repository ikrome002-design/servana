# Screen specification — Platform feature flags
...
- **Screen key:** `platform-feature-flags`
- **Route name and URL:** `platform.feature-flags`
- **Layout:** `PlatformAdminLayout`
- **Allowed roles:** `super_administrator`
- **Required permissions:** `platform.settings.view`, `platform.settings.update` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Contract page §5.4.20 (COR-UI08-001). Maker and checker feature-flag governance with an append-only history. A flag can never grant a permission, bypass an entitlement, bypass a billing state or open an external gate: it only enables or disables an already-authorized capability.
