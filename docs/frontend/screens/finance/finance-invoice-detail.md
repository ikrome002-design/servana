# Screen specification — Invoice detail

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase 17**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `finance-invoice-detail`
- **Route name and URL:** `finance.invoices.detail`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `invoice.view`, `invoice.void.request_or_execute_as_policy`, `invoice.adjustment.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance invoice detail with the additive void/adjust workflow: capability-gated Void (request → confirm execute / reject) and Adjust controls, each requiring a mandatory reason and showing an irreversible-action warning. Finance MFA + fresh step-up on void are enforced by the API; original snapshots and the number are never rewritten.
