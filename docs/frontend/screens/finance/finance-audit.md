...
- **Screen key:** `finance-audit`
- **Route name and URL:** `finance.audit`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `finance.audit.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** The Finance role's own MFA-gated, branch-scoped, masked read of the finance-domain audit trail (finance.audit.view; distinct from the Audit role's audit.finance.view). Read-only: no operational mutation and no flagged-review controls; MFA is enforced by the API and reflected in the UI.
