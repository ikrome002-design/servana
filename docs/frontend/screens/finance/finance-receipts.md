...
- **Route name and URL:** `finance.receipts`
...
- **Fields and displayed data:** Finance receipt list (branch-scoped): receipt number, invoice, server-formatted KES amount, reissue badge, and readiness. Receipts are issued AUTOMATICALLY on validation; there is no manual issue action.
