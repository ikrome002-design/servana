# Screen specification — Cash-up review
...
- **Screen key:** `finance-cash-up`
- **Route name and URL:** `finance.cash-up`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `cash_up.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance cash-up review inbox (branch-scoped): business date, server-derived expected + counted totals, and variance per submitted cash-up. Opens the review detail.
