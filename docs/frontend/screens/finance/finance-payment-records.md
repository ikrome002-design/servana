...
- **Screen key:** `finance-payment-records`
- **Route name and URL:** `finance.payment-records`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `customer_payment.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance read-only list of payment recording groups awaiting action (branch-scoped, masked): invoice number, maker, total, and status (pending validation / held for duplicate review). No validation, rejection, receipt, or refund control (Phase 18B).
