# Screen specification — Refund detail
...
- **Screen key:** `finance-refund-detail`
- **Route name and URL:** `finance.refunds.detail`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `refund.create`, `refund.approve`, `refund.finalize` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Refund detail showing the refunded component, masked external reference, maker/checker state, and status. A DISTINCT Finance membership approves + finalizes (both require a fresh step-up, server-enforced); finalization is IRREVERSIBLE and reduces the recognised invoice balance, with an explicit warning. Period-lock (423) errors are surfaced verbatim.
