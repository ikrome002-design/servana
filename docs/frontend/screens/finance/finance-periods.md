...
- **Screen key:** `finance-periods`
- **Route name and URL:** `finance.periods`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `period_lock.create`, `period_lock.reopen` (frontend visibility only; backend EnsurePermission + policy is authoritative)
