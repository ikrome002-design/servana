# Screen specification — Cash-up review detail
...
- **Screen key:** `finance-cash-up-detail`
- **Route name and URL:** `finance.cash-up.detail`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `cash_up.view`, `cash_up.approve`, `cash_up.reject`, `cash_up.request_correction` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Cash-up detail with per-method expected/counted/variance lines. Finance (checker) approves, rejects, or requests correction of a submitted cash-up, and locks an approved one. The branch manager who submitted can never approve (backend actor guard). Expected totals are server-derived; the submitted snapshot is not overwritten.
