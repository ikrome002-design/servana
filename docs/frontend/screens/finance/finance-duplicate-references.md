...
- **Screen key:** `finance-duplicate-references`
- **Route name and URL:** `finance.payments-duplicates`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `customer_payment.duplicate_override` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Assigned-branch duplicate-reference queue using masked normalized comparisons and server-owned risk facts, linking to the existing MFA and fresh-step-up override workflow without exposing raw references.
