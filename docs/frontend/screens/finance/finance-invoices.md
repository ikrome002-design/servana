...
- **Route name and URL:** `finance.invoices`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `invoice.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance invoice list (branch-scoped, masked client): invoice number/'Draft', total, and status badge. Finance holds invoice.view but NOT invoice.create — no 'New invoice' button is shown.
