...
- **Screen key:** `finance-refunds`
- **Route name and URL:** `finance.refunds`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `refund.create` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance external-refund list with a status filter and a capability-gated 'Request refund' form (against a validated component; amount capped server-side at the remaining refundable; masked external reference). Servana never moves funds — a refund records the intent.
