# Screen specification — Receipt detail
...
- **Screen key:** `finance-receipt-detail`
- **Route name and URL:** `finance.receipts.detail`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `receipt.view`, `receipt.reissue` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Immutable receipt + snapshot components. Finance may REISSUE (a new gap-free number referencing the original; mandatory reason) and DOWNLOAD via a short-lived authorized signed link (used immediately, never stored). No manual issue action.
