...
- **Screen key:** `finance-exports`
- **Route name and URL:** `finance.exports`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `finance_export.create`, `finance_export.download` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Finance exports: request a scoped, masked export (a fresh step-up is server-enforced; only invoices/payments/receipts/cash_up/refunds/disputes are offered — compensation/payouts/billing are not), track status, download a ready export via a short-lived authorized signed link (never stored), and revoke. Download accounting is server-side.
