# Screen specification — Dispute detail
...
- **Screen key:** `finance-dispute-detail`
- **Route name and URL:** `finance.disputes.detail`
- **Layout:** `FinanceLayout`
- **Allowed roles:** `merchant_finance`
- **Required permissions:** `finance_dispute.manage` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Move a dispute open → under_review → resolved/rejected (resolution/rejection need a mandatory note). The linked invoice/payment is READ-ONLY here; private evidence downloads via an authorized signed link.
