# Screen specification — Magic Link verify

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase 5**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `auth-verify`
- **Route name and URL:** `auth.verify`
- **Layout:** `AuthLayout`
...
- **Fields and displayed data:** Consumes a Magic Link token and routes to setup or the role landing.
