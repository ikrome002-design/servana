# Screen specification — Access denied

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase UI-03**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `auth-access-denied`
- **Route name and URL:** `access-denied`
- **Layout:** `none`
...
- **Fields and displayed data:** Role-safe denial state reached by the account-entry guard. Names no account or resource, never redirects to another account, and offers only sign-out plus the contexts the server says the user may enter.
