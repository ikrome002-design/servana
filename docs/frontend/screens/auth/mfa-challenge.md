# Screen specification — MFA / step-up challenge

> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase R3**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `mfa-challenge`
- **Route name and URL:** `auth.mfa.challenge`
- **Layout:** `AuthLayout`
- **Allowed roles:** `all`
...
- **Fields and displayed data:** Per-session MFA / step-up challenge via TOTP or recovery code.
