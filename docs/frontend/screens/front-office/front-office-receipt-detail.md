...
- **Fields and displayed data:** Front Office receipt detail: immutable receipt + snapshot components with a DOWNLOAD action (short-lived authorized signed link, never stored). No reissue control appears for Front Office (it requires receipt.reissue, which Front Office does not hold).
