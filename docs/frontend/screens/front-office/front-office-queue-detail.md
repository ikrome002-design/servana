# Screen specification — Queue entry detail
...
- **Screen key:** `front-office-queue-detail`
- **Route name and URL:** `front-office.queue.detail`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `queue.view`, `queue.assign`, `queue.transfer` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Queue-entry detail with capability-gated actions as dialogs: assign/transfer eligible personnel (reason required for transfer), call, start, complete, mark no-show, and cancel (reason required). Status badge + controls driven by the API capability map; client contact is masked.
