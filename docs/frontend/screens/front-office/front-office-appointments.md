...
- **Screen key:** `front-office-appointments`
- **Route name and URL:** `front-office.appointments`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `appointment.view` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Front Office appointment list for a chosen business date (branch-scoped): client, service, assigned personnel, status badge, start/end time, with date navigation and status filtering. Create button gated by appointment.create. Walk-ins/queue/sessions are later phases and are not shown here.
