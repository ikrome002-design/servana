# Screen specification — Book appointment
...
- **Screen key:** `front-office-appointment-create`
- **Route name and URL:** `front-office.appointments.create`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `appointment.create` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Book an appointment: pick a branch-scoped client and active service, choose a start time (server-calculated duration/end-time preview), optionally pick a preferred and/or assigned eligible personnel member with availability/conflict feedback. The backend derives merchant/branch/end-time/status; no fee is calculated.
