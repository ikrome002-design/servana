# Screen specification — Queue board
...
- **Screen key:** `front-office-queue`
- **Route name and URL:** `front-office.queue`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `queue.view`, `queue.create`, `queue.assign`, `queue.transfer`, `queue.reorder` (frontend visibility only; backend EnsurePermission + policy is authoritative)
