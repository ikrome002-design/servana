...
> Generated from `docs/frontend/screens/inventory.json` (Plan §27.1). Status: **implemented** · Owning phase: **Phase UI-13**. Edit the inventory + regenerate (`node scripts/generate-screen-specs.mjs`); the owning phase writes the final detailed spec before implementing future behavior.

- **Screen key:** `front-office-queue-transfer`
- **Route name and URL:** `front-office.queue-transfer`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `queue.view`, `queue.transfer` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Dedicated transfer screen retaining the queue subject beside a server-filtered eligible-personnel form; reason, branch scope and queue state remain backend-enforced.
