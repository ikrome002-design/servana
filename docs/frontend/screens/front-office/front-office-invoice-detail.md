...
- **Screen key:** `front-office-invoice-detail`
- **Route name and URL:** `front-office.invoices.detail`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `invoice.view`, `invoice.create` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Invoice detail with masked client, itemized services, a separately-shown preferred-personnel fee, subtotal/discount/tax/total, validated-paid and balance. A draft shows a capability-gated Finalize action (confirmation: allocates the number and locks amounts); finalized invoices show a read-only snapshot note. No payment or receipt control appears here.
