# Screen specification — Start a walk-in
...
- **Screen key:** `front-office-walk-in`
- **Route name and URL:** `front-office.walk-ins`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `queue.create` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Accessible walk-in wizard: find an existing branch client or create one through the existing client workflow, select an active service, choose the assignment mode, select personnel where required, and confirm. Creates the walk-in and queue entry atomically; the wait estimate is server-calculated and labelled. No preferred-personnel fee is shown or charged.
