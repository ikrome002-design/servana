...
- **Screen key:** `front-office-clients`
- **Route name and URL:** `front-office.clients`
- **Layout:** `FrontOfficeLayout`
- **Allowed roles:** `merchant_front_office`
- **Required permissions:** `client.view`, `front_office.search` (frontend visibility only; backend EnsurePermission + policy is authoritative)
...
- **Fields and displayed data:** Front Office client directory + search by name or phone (branch/tenant-scoped). Contact is ALWAYS masked (phone last-four, masked email); the blind index is never shown.
