...
/**
 * Return a submitted cash-up to the Branch Manager for correction (Plan §45; Phase
 * 18B). Finance checker action: `submitted → correction_requested`. Requires a
 * mandatory reason; maker/checker separation (requester != submitter →
 * `403 maker_is_checker`). Opens a new correction cycle rather than a silent rewrite.
 * Period gate → `423`; invalid source state → `422 invalid_state_transition`.
 */
final class RequestCashUpCorrection
{
    ...
    public function handle(BranchCashUp $cashUp, User $checker, string $reason): BranchCashUp
    {
        ...
        return DB::transaction(function () use ($cashUp, $checker, $reason): BranchCashUp {
            ...
            $this->machine->ensure($locked->status, CashUpStatus::CorrectionRequested);

            $locked->forceFill([
                'status' => CashUpStatus::CorrectionRequested->value,
                'reviewed_by' => $checker->id,
                'reviewed_at' => CarbonImmutable::now(),
                'review_note' => $reason,
            ])->save();

            $this->audit->record(AuditEvent::CashUpCorrectionRequested, $checker, $locked->merchant_id, $locked->branch_id, $locked, [
