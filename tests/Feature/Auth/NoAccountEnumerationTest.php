...
it('returns identical responses whether or not the email exists (request)', function (): void {
    ...
    $existing = postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'real@salon.co.ke']);
    $missing = postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'fake@salon.co.ke']);
...
it('returns identical failures for inactive-user vs nonexistent-token (verify)', function (): void {
    ...
    $raw = issueBoundMagicLink('real@salon.co.ke');
    $user->status = User::STATUS_SUSPENDED; // not mass-assignable (security)
    $user->save();
    $ineligible = postOnHost('merchant_administrator', '/api/v1/auth/magic-link/verify', ['token' => $raw]);

    // Case B: a token that never existed.
    $bogus = postOnHost('merchant_administrator', '/api/v1/auth/magic-link/verify', ['token' => 'no-such-token']);
