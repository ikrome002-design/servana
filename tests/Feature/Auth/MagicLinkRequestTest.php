...
it('returns a uniform 202 for an eligible user and sends the link', function (): void {
    ...
    postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'owner@salon.co.ke'])
        ->assertStatus(202)
...
it('returns the same 202 and sends nothing when the email does not exist', function (): void {
    ...
    postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'ghost@nowhere.co.ke'])
        ->assertStatus(202)
        ->assertJsonPath('message', 'If the email exists and is active, a link was sent.');
...
it('sends nothing for a suspended user but still returns 202', function (): void {
    ...
    postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'suspended@salon.co.ke'])
        ->assertStatus(202);
...
it('rejects an invalid email with a structured validation envelope', function (): void {
    postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'not-an-email'])
        ->assertStatus(422)
...
it('throttles repeated requests for the same email with a structured 429', function (): void {
    ...
    for ($i = 0; $i < 3; $i++) {
        postOnHost('merchant_administrator', '/api/v1/auth/magic-link', ['email' => 'busy@salon.co.ke'])
            ->assertStatus(202);
    }
