#!/usr/bin/env node
/** Phase UI-11 canonical-host proof against the final production image pair. */
import http from 'node:http';
import { readFileSync } from 'node:fs';

const [, , origin = 'http://127.0.0.1:8111'] = process.argv;
...
const activation = fromRoot('docs/frontend/audits/ui-11/route-activation-matrix.json');
const gates = fromRoot('docs/frontend/audits/ui-11/gate-disposition.json');
const staff = '01HZZSTAFF0000000000000000';
const materialize = (path) => path.replace(':staffUlid', staff);
const hostFor = (key) => {
...
const hrHost = hostFor('merchant_human_resource');
...
console.log(`Human Resource host: ${hrHost}`);
let sharedAssets = [];
for (const path of implemented) {
  const response = await request(path, hrHost);
  const account = embeddedAccount(response.body);
  check(`implemented ${path}`, response.status === 200 && String(response.headers['content-type']).includes('text/html') && account === 'merchant_human_resource', `status ${response.status}, account ${account}`);
...
for (const asset of sharedAssets) {
  const response = await request(asset, hrHost);
...
if (script) {
  const sourceMap = await request(`${script}.map`, hrHost);
  check('no production JavaScript source map', sourceMap.status === 404, `status ${sourceMap.status}`);
}
for (const path of gated) {
  const response = await request(path, hrHost);
  check(
    `gated ${path} has only the ordinary Human Resource shell`,
    response.status === 200 && embeddedAccount(response.body) === 'merchant_human_resource'
      && !/profile updated|role assigned|branch assigned|report catalogue|notification inbox/i.test(response.body),
    `status ${response.status}`,
  );
}
for (const path of compatibility) {
  const response = await request(path, hrHost);
  check(`compatibility ${path} stays on the Human Resource host`, response.status === 200 && embeddedAccount(response.body) === 'merchant_human_resource', `status ${response.status}`);
}
for (const [accountKey, path] of [['merchant_administrator', '/dashboard'], ['merchant_audit', '/audit'], ['merchant_finance', '/account']]) {
  const response = await request(path, hostFor(accountKey));
  const account = embeddedAccount(response.body);
  check(`${path} on ${accountKey} host never serves HR context`, response.status === 200 && account === accountKey, `account ${account}`);
}
