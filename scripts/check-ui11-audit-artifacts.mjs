/** Phase UI-11 evidence consistency checker. */
...
const audit = join(root, 'docs/frontend/audits/ui-11');
const required = [
...
const pages = nav.pages.filter((page) => page.account_type === 'merchant_human_resource');
const byKey = new Map(pages.map((page) => [page.screen_key, page]));
if (pages.length !== 19) problems.push(`canonical HR count must be 19, found ${pages.length}`);
const counted = pages.reduce((totals, page) => {
  totals[page.implementation_status] = (totals[page.implementation_status] ?? 0) + 1;
  return totals;
}, {});
if (counted.implemented !== 15 || counted.disabled_by_gate !== 4 || (counted.planned ?? 0) !== 0 || (counted.removed_by_authority ?? 0) !== 0) {
  problems.push('canonical HR navigation is not 15 implemented / 4 gated / 0 planned / 0 removed');
}

if (existsSync(join(audit, 'page-readiness-matrix.json'))) {
  const readiness = json('page-readiness-matrix.json');
  if (readiness.entries.length !== 19 || readiness.class_counts.F !== 0) problems.push('readiness must contain 19 rows and no class-F ambiguity');
  if (readiness.target_disposition.implemented !== 15 || readiness.target_disposition.disabled_by_gate !== 4 || readiness.target_disposition.planned !== 0 || readiness.target_disposition.removed_by_authority !== 0) problems.push('readiness disposition mismatch');
}
if (existsSync(join(audit, 'route-activation-matrix.json'))) {
  const routes = json('route-activation-matrix.json');
  if (routes.implemented_routes.length !== 15 || routes.gated_no_route.length !== 4) problems.push('route activation must name 15 live routes and 4 gated no-routes');
...
if (existsSync(join(audit, 'gate-disposition.json'))) {
  ...
  if (!gates.external_gate_w_closed || !gates.phase_21n_closed || gates.entries.length !== 4 || gates.entries.some((entry) => entry.route_exists || entry.component_exists || entry.network_runtime_exists)) problems.push('Gate disposition must remain closed with exactly four inert entries');
}
...
if (existsSync(join(audit, 'accessibility-matrix.json'))) {
  ...
  if (a11y.axe.light_pages.length !== 15 || a11y.axe.serious !== 0 || a11y.axe.critical !== 0) problems.push('accessibility proof must cover 15 pages with zero serious/critical');
}
if (existsSync(join(audit, 'browser-proof.json'))) {
  const browser = json('browser-proof.json');
  if (browser.focused_playwright.passed !== 47 || browser.focused_playwright.failed !== 0 || browser.focused_playwright.exit_code !== 0) problems.push('focused browser proof must record 47/47');
  const whole = browser.whole_product_playwright;
  if (whole.collected !== 1393 || whole.passed !== 1393 || whole.failed !== 0 || whole.flaky !== 0 || whole.skipped !== 0 || whole.exit_code !== 0) problems.push('whole-product browser proof must record 1393/1393 with no flaky/skipped cases');
}
if (existsSync(join(audit, 'production-host-proof.json'))) {
  const production = json('production-host-proof.json');
  if (production.images.length !== 2 || production.images.some((image) => image.build_exit_code !== 0)) problems.push('both invalidated production images must build');
...
if (existsSync(join(audit, 'defect-closure.json'))) {
  ...
  if (closure.closures.some((item) => item.lifecycle !== 'verified_complete') || closure.verified_complete !== closure.closures.length || closure.local_complete !== 0) problems.push('all UI-11 closures must be verified_complete after PR #61 merge reconciliation');
}
if (existsSync(join(audit, 'screenshot-index.json'))) {
  const screenshots = json('screenshot-index.json');
  if (screenshots.captures.filter((capture) => capture.screen_key !== null).length < 15) problems.push('screenshots must cover all implemented pages');
...
if (problems.length) {
  console.error('UI-11 audit artifacts FAILED:');
  for (const problem of problems) console.error(`  ${problem}`);
  process.exit(1);
}
console.log('UI-11 audit artifacts: OK — 19 pages, 15 implemented, 4 gated, 0 planned, permission catalogue 169/134/35, OpenAPI 344 operations.');
